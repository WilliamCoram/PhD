...
Newton-polygon theory (`PhD/Test/test.lean`) consumes them at the *slope radii* `c = exp m`,
...
* **Rung 3 (limits): every `c > 0` — not pursued.**  The divisible closure of a nontrivial
  ...
  (`NewtonPolygon.memDivisibleValueGroup_exp_slope` in `PhD/Test/test.lean`).
...
`NewtonPolygon.exists_factor_of_distinguished` in `PhD/Test/test.lean` — currently stated
...
`PhD/Test/test.lean` consumes.  The base-change and descent arguments are
...
/-! ## §6  Bridge to the Newton-polygon file (to be done in `PhD/Test/test.lean`, not here)
