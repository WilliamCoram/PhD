...
1. the **Martin proof** — elementary, extension-free, maximally general; this is the
   *canonical* development and lives in `PhD/ForMathlib/`;
2. the **Berkovich proof** — via the Gauss-extension field `K_r = ℋ(η_r)` and descent; the
   classical route, kept for its reusable objects; it lives in `PhD/BirkovichWP/`.
...
## 1. The Martin proof (canonical, `PhD/ForMathlib/`)
...
| File (`PhD/ForMathlib/RingTheory/…` unless noted) | Contents |
|---|---|
...
`PhD/ForMathlib` imports only `Mathlib` / `PhD.ForMathlib` / `PhD.Mathlib`.

---

## 2. The Berkovich proof (legacy, `PhD/BirkovichWP/`)
...
### Lean file map (legacy, `PhD/BirkovichWP/RingTheory/…`)
...
mathlib-bound version in `PhD/ForMathlib/`. The **Berkovich** one (Gauss extensions
`ℋ(η_r)` + descent) proves the same statements over a field by the classical reduction and
is retained under `PhD/BirkovichWP/` for the reusable `GaussExtension` field, the descent
