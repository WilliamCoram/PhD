...
**H1b**, being worked in `PhD/Test/AtkinLehnerIdentity.lean`.  `AtkinLehnerHypothesis` is exactly
`H1a ∧ H1b`.
...
* **H1b** the operator identity `A B = p^{k+1}` (worked in `PhD/Test/AtkinLehnerIdentity.lean`), and
