# `PhD/LWX/` — reading order and file guide
...
imports only files of this folder with a strictly smaller `NN` (plus modules from `PhD/TateFredholm/`, `PhD/QMF/`, `PhD/NewtonPolygons/` and `PhD/ForMathlib/`).  Files that share a number
