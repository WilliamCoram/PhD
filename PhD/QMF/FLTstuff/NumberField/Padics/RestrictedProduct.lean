...
PORT (T016e-2) of `FLT.NumberField.Padics.RestrictedProduct` (158 l.).
-/
...
theorem padicNatDen_norm_of_mem {x : Πʳ (p : Nat.Primes), [ℚ_[p], subring p]} {p : Nat.Primes}
  ...
  · intro q hq hpq
    ...
    · simp
    -- PORT (v4.33): `Subtype.val_inj` no longer applies to the `Nat.Primes → ℕ` coercion;
    -- use the bespoke `Nat.Primes.coe_nat_inj` instead (here and in the two lemmas below).
    · exact (Nat.coprime_primes p.2 q.2).2 (by simpa [Nat.Primes.coe_nat_inj] using! hpq.symm)
...
theorem padicNatDen_norm_of_notMem {x : Πʳ (p : Nat.Primes), [ℚ_[p], subring p]} {p : Nat.Primes}
  ...
  · exact (Nat.coprime_primes p.2 q.2).2
      -- PORT (v4.33): `Subtype.val_inj` → `Nat.Primes.coe_nat_inj`.
      (by simpa [Nat.Primes.coe_nat_inj] using! ne_of_mem_of_not_mem hq hp |>.symm)
...
theorem padic_exists_sub_mem_padicInt
  ...
  have ha : Pairwise fun i j ↦ (a i).Coprime (a j) :=
    -- PORT (v4.33): `Subtype.val_inj` alone no longer closes this; the `Nat.Primes → ℕ`
    -- coercion needs `Nat.Primes.coe_nat_inj` as well.
    fun p q h ↦ Nat.coprime_pow_primes _ _ p.1.2 q.1.2
      (by simpa [Nat.Primes.coe_nat_inj, Subtype.val_inj] using! h)
