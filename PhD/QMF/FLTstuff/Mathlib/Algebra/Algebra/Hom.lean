...
`PhD.QMF.FLTstuff.DedekindDomain.Completion.BaseChange` and are omitted.

-- PORT (T016d-2b extension): `SemialgHom.compAlgHom`, `SemialgHom.prod` and
`SemialgHom.prodMap` appended (needed by
`PhD.QMF.FLTstuff.Mathlib.Topology.Algebra.Algebra.Hom`); the file is now a full port.
-/
