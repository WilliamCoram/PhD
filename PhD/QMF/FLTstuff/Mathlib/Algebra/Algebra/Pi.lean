...
-/
import PhD.QMF.FLTstuff.Mathlib.Algebra.Algebra.Hom
...
PARTIAL PORT (T016d part 2a) of `FLT.Mathlib.Algebra.Algebra.Pi`: only `Pi.semialgHom` and
`Pi.semialgHom_apply` are needed by `PhD.QMF.FLTstuff.DedekindDomain.Completion.BaseChange`;
the rest of the FLT file (the
`RingHom.piIndex` / `Pi.ringHomEquivOfIsDomain` / `Pi.algHomEquivOfIsDomain` block) is omitted.

-- PORT (T016e extension): `Pi.semialgHomPi` (with its `_apply` lemma) and
`AlgEquiv.piCongrFiberwise` appended from the same FLT source file (needed by
`PhD.QMF.FLTstuff.NumberField.Completion.Infinite` and
`PhD.QMF.FLTstuff.NumberField.InfiniteAdeleRing`).
-/
