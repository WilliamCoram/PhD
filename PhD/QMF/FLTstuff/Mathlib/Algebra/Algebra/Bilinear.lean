...
`PhD.QMF.FLTstuff.DedekindDomain.Completion.BaseChange` and were omitted.
...
-- PORT (T016f-2a extension): `PhD.QMF.FLTstuff.Mathlib.LinearAlgebra.Determinant` and
-- `PhD.QMF.FLTstuff.HaarMeasure.HaarChar.FiniteDimensional` need the `LinearEquiv.mulLeft` /
