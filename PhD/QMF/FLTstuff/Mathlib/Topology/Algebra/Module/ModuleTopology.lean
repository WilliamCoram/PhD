...
-- PORT (v4.33): for the `Module.Free.of_divisionRing` instance used by `t2Space'`.
import Mathlib.LinearAlgebra.Basis.VectorSpace
import PhD.QMF.FLTstuff.Mathlib.Algebra.Algebra.Tower
-- PORT (T016e extension): for the `IsTopologicalModule` class used by the `trans` section.
import PhD.QMF.FLTstuff.Deformations.ContinuousRepresentation.IsTopologicalModule
-- PORT (T016f-1 extension): for `Module.finBasisOfFinrankEq` used by `Module.Basis.equivFunHomeo`.
...
PARTIAL PORT (T016a) of `FLT.Mathlib.Topology.Algebra.Module.ModuleTopology`: only the
declarations needed by `PhD.QMF.FLTstuff.Mathlib.Topology.Algebra.RestrictedProduct.Module`
are ported (`ModuleTopology.iff`, `IsModuleTopology.of_isQuotientMap`,
`IsModuleTopology.of_isOpenMap_surjective`); the rest of the FLT file is not needed here and
its mathlib-available lemmas are used directly from Mathlib.

EXTENDED (T016d part 2a): `IsModuleTopology.continuousLinearEquiv`,
`IsModuleTopology.continuousAlgEquivOfIsBiscalar` (with its `_apply` lemma) and
`IsModuleTopology.t2Space` / `t2Space'` are additionally ported from the same FLT source
file for `PhD.QMF.FLTstuff.DedekindDomain.Completion.BaseChange`.

-- PORT (T016d-2b extension): `IsModuleTopology.continuousAlgEquivOfAlgEquiv` appended
(needed by `PhD.QMF.FLTstuff.DedekindDomain.FiniteAdeleRing.BaseChange`).

-- PORT (T016e extension): appended from the same FLT source file — the `opensubring`
section (`of_continuous_isOpenMap_algebraMap`), `continuousLinearEquivOfIsBiscalar`, the
`trans` section (`isTopologicalModule`, `Algebra.moduleTopology_le`, `moduleTopology.trans`,
`IsModuleTopology.trans`) and the `Prod` section (`Prod.leftAlgebra` … `instProd'`), needed by
`PhD.QMF.FLTstuff.NumberField.{Completion.Infinite, InfiniteAdeleRing, AdeleRing}`.

-- PORT (T016f-1 extension): appended the `ModuleFinite` section
(`IsModuleTopology.Module.Basis.equivFunHomeo`) from the same FLT source file, needed by
`PhD.QMF.FLTstuff.HaarMeasure.HaarChar.Ring`.

-- PORT (T016f-2b extension): appended the `locally_compact` section
(`IsModuleTopology.locallyCompactSpaceOfFinite`) from the same FLT source file, needed by
`PhD.QMF.FLTstuff.HaarMeasure.HaarChar.FiniteAdeleRing`.

-- PORT (T016g extension): appended the `SecondCountableTopology` section
(`Module.Finite.secondCountabletopology`) from the same FLT source file, needed by
`PhD.QMF.FLTstuff.DivisionAlgebra.Finiteness`.
-/

-- PORT (T016e extension): `ModuleTopology.isModuleTopology` appended from the FLT file.
theorem ModuleTopology.isModuleTopology (R : Type*) [TopologicalSpace R] (S : Type*) [Add S]
    [SMul R S] : @IsModuleTopology R _ S _ _ (moduleTopology R S) :=
...
-- PORT (T016d-2b extension): appended from the FLT file.
...
theorem of_continuous_isOpenMap_algebraMap (hcont : Continuous (algebraMap R S))
  ...
  eq_moduleTopology' := by
    ...
    rw [IsTopologicalRing.to_topologicalAddGroup.ext_iff <|
      ...
      @IsModuleTopology.topologicalAddGroup R _ _ S _ _ (moduleTopology R S)
        (ModuleTopology.isModuleTopology R S)]
...
theorem isTopologicalModule
    ...
    [IsModuleTopology R M] : IsTopologicalModule R M where
      continuous_smul := eq_moduleTopology R M ▸ (ModuleTopology.continuousSMul R M).1
      continuous_add := eq_moduleTopology R M ▸ (ModuleTopology.continuousAdd R M).1
...
lemma _root_.Algebra.moduleTopology_le [IsTopologicalModule R S] :
  ...
  haveI : ContinuousAdd M := ModuleTopology.continuousAdd S M
  have ⟨cts_smul⟩ : ContinuousSMul S M := ModuleTopology.continuousSMul S M
...
lemma _root_.moduleTopology.trans [IsTopologicalRing R] [Module.Finite R S] [IsModuleTopology R S] :
  ...
  haveI : IsModuleTopology R M := ModuleTopology.isModuleTopology R M
  haveI : ContinuousAdd M := ModuleTopology.continuousAdd R M
  have ⟨cts_smul⟩ : ContinuousSMul R M := ModuleTopology.continuousSMul R M
...
lemma trans [IsTopologicalRing R] [Module.Finite R S] [IsModuleTopology R S]
  ...
  simp [ModuleTopology.iff R M, ModuleTopology.iff S M, moduleTopology.trans R S]
...
-- PORT (T016f-2b extension): the `locally_compact` section from the FLT source file
-- (`IsModuleTopology.locallyCompactSpaceOfFinite`), needed by
-- `PhD.QMF.FLTstuff.HaarMeasure.HaarChar.FiniteAdeleRing`.
...
-- PORT (T016f-1 extension): the `ModuleFinite` section from the FLT source file
-- (`Module.Basis.equivFunHomeo`), needed by `PhD.QMF.FLTstuff.HaarMeasure.HaarChar.Ring`.
...
-- PORT (T016g extension): the `SecondCountableTopology` section from the FLT source file
-- (`Module.Finite.secondCountabletopology`), needed by
-- `PhD.QMF.FLTstuff.DivisionAlgebra.Finiteness`.
