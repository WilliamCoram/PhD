...
PORT (T016f-1) of `FLT.Mathlib.MeasureTheory.Group.Action`.
-/
...
-- PORT (v4.33): mathlib now has `MeasureTheory.Measure.IsMulLeftInvariant.comap`, but it assumes
-- `Group H` whereas this FLT version only needs `Monoid H`, so the FLT proof is retained.
@[to_additive]
...
-- PORT (v4.33): mathlib now has `MeasureTheory.Measure.IsMulRightInvariant.comap`, but it assumes
-- `Group H` whereas this FLT version only needs `Monoid H`, so the FLT proof is retained.
@[to_additive]
