...
-/
import PhD.QMF.FLTstuff.HaarMeasure.MeasurableSpacePadics
import PhD.QMF.FLTstuff.HaarMeasure.HaarChar.Ring
import PhD.QMF.FLTstuff.Mathlib.MeasureTheory.Group.Action
import PhD.QMF.FLTstuff.Mathlib.NumberTheory.Padics.PadicIntegers
...
PORT (T016f-2a) of `FLT.HaarMeasure.HaarChar.Padic`.

NOTE: this file takes its `MeasurableSpace ℚ_[p]` instance from
`PhD.QMF.FLTstuff.HaarMeasure.MeasurableSpacePadics`; it must never be co-imported with
`PhD.QMF.FLTstuff.Mathlib.MeasureTheory.Constructions.BorelSpace.Padic`, which declares a
competing one.
-/
...
lemma MeasureTheory.ringHaarChar_padic (x : ℚ_[p]ˣ) : ringHaarChar x = ‖(x : ℚ_[p])‖₊ := by
  ...
  ext x
  -- PORT (v4.33): FLT's simp set ends with `Units.val_mk0`, which no longer fires here: `ℤ_[p]`
  -- unfolds to a subtype of `ℚ_[p]`, so `↑↑x` inside `Units.mk0` is not type-correct at
  -- `implicit` transparency and simp cannot rewrite under it. Drop it and finish with a term,
  -- which is checked at default transparency.
  simp only [MonoidHom.coe_coe, Function.comp_apply, MonoidHom.coe_mk,
    OneHom.coe_mk, coe_nnnorm, PadicInt.padic_norm_e_of_padicInt,
    ringHaarChar_padic_padicInt]
  exact (PadicInt.padic_norm_e_of_padicInt _).symm
