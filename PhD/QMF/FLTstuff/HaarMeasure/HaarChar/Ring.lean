...
import PhD.QMF.FLTstuff.Mathlib.MeasureTheory.Constructions.BorelSpace.RestrictedProduct
import PhD.QMF.FLTstuff.Mathlib.Topology.Algebra.RestrictedProduct.Equiv
import Mathlib.MeasureTheory.Measure.Haar.MulEquivHaarChar
import PhD.QMF.FLTstuff.HaarMeasure.HaarChar.AddEquiv
-- PORT (T016f-1): for `RestrictedProduct.unitsEquiv`, mathlib's version of FLT's
...
PORT (T016f-1) of `FLT.HaarMeasure.HaarChar.Ring`.

SUBSTITUTION: FLT's `MulEquiv.restrictedProductUnits` has landed in mathlib as
`RestrictedProduct.unitsEquiv` (`Mathlib.Topology.Algebra.RestrictedProduct.Units`), so
`ringHaarChar_restrictedProduct` is stated with the mathlib name instead.
-/
...
-- PORT (v4.33): `MulEquiv.restrictedProductUnits` is now mathlib's
-- `RestrictedProduct.unitsEquiv A`.
lemma ringHaarChar_restrictedProduct (u : (Πʳ i, [A i, C i])ˣ) :
    ringHaarChar u = ∏ᶠ i, ringHaarChar (RestrictedProduct.unitsEquiv A u i) := by
  set u := RestrictedProduct.unitsEquiv A u
