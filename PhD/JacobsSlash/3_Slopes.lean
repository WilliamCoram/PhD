...
import PhD.JacobsSlash.«1_SlopeTheorem»
...
and `ω` are documented where they are introduced, in `PhD.JacobsSlash.U3Data`'s module
...
* the **identification** — the transcribed matrix is the matrix of `U₃`
  ...
  in `PhD.JacobsSlash.U3.Matrix`; see `PhD.JacobsSlash.U3Data`'s module header for the full list of
  formalised inputs, Thm 2.1 – Prop 2.6);
* the **factorisation** — `M₂,₂` is the middle factor of
  `det(1 − T·U₃) = det(1 − T·M₁,₁)·det(1 − T·M₂,₂)·det(1 − T·M₃,₃)`
  (`JacobsSlash.charPowerSeries_U3MatrixOp` in `PhD.JacobsSlash.DiamondW`, via [Jacobs, Lemma 2.10]
...
`PhD.JacobsSlash.DiamondW`'s module header).  Nothing downstream depends on it: the
...
(the AG-NP tranche, `PhD.JacobsSlash.SlopeReading`), which is where [Jacobs, Corollary 2.16]
is stated in full.
-/
...
valuation but does not generate the value group.  See the `PhD.JacobsSlash.U3Data` module header. -/
