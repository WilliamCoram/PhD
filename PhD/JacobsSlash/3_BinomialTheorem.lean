...
with `(1 + z)^t = exp₃(t · log₃(1 + z))` — `JacobsSlash.unitPow` — on the right.  This file
...
* `JacobsSlash.unitPow_add`, `JacobsSlash.unitPow_one_eq`, `JacobsSlash.unitPow_natCast`: the weight power
  is additive in the exponent, is the identity at `t = 1`, and is the ordinary power at
  natural exponents.
* `JacobsSlash.binomialCoeff_natCast`: `(m choose n)` at a natural exponent is the ordinary
  binomial coefficient — in particular it vanishes for `n > m`.
* `JacobsSlash.tsum_binomialCoeff_natCast`: the binomial theorem at natural exponents.
* `JacobsSlash.norm_unitPow_sub_unitPow_le`, `JacobsSlash.norm_tsum_binomialCoeff_sub_le`: both sides
  are `‖t − t'‖`-Lipschitz in the exponent.
* `JacobsSlash.tsum_binomialCoeff_eq_unitPow`: the binomial theorem, for any exponent
  approximable by natural numbers.
* `JacobsSlash.binomialSeries_ode`: the binomial series satisfies `(1 + e·x)·B′ = t·e·B` — the
