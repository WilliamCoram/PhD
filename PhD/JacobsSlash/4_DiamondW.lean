...
`PhD.JacobsSlash.U3Data`, zero diagonal), the diamond operator
...
lemma (`PhD.JacobsSlash.BlockOp`) this yields the milestone
...
characteristic power series as `U3MatrixOp` (both in `PhD.JacobsSlash.U3.Matrix`; the
formalised inputs Thm 2.1 – Prop 2.6 are listed in `PhD.JacobsSlash.U3Data`'s module header).
...
§B.2 factorisations) would need the certificate machinery of `PhD.JacobsSlash.U3.Factorisations`
...
`PhD.JacobsSlash.U3Data`, exactly as the thesis argues it ("since `U₃` and `W` commute, we
...
Exactly as for `ε₀,₂`, `ε₁,₂` in `PhD.JacobsSlash.U3Data`: each matrix `(a b; c d)` is fed to
...
`JacobsSlash.U3.charPowerSeries_blockOp_eq_U3MatrixOp` (`PhD.JacobsSlash.U3.Matrix`; module
header). -/
...
hence every slope statement of this file and `PhD.JacobsSlash.Slopes`, is identical for the
...
certificates of `PhD.JacobsSlash.U3.«7_DiamondHecke»` index off *this* table, so
...
`PhD.JacobsSlash.U3.«7_DiamondHecke»`'s `heckeW_apply_classRep_eq_delta`, this is what
...
`norm_coeff_M22genFun_le` (`PhD.JacobsSlash.U3Data`) with the scalars `ω²`, `ω` replaced by
...
`H₃,₃` row-integral (`‖ω‖ = ‖ω²‖ = 1`), exactly as for `M22op` in `PhD.JacobsSlash.U3Data`. -/
...
(proved in `PhD.JacobsSlash.U3Data`). -/
theorem lemma210 :
