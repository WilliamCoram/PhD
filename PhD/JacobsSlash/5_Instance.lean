...
The case-B development (`PhD.JacobsSlash.Slopes`, `PhD.JacobsSlash.SlopeReading`) is stated over an
...
* **The half-integer slopes are forced there, not here.**  Over `ℚ₃(ζ₃)` the extension is
  ...
  [Jacobs, Cor 2.16] are *compelled* to be half-integers (see the `PhD.JacobsSlash.U3Data` module
...
(`PhD.JacobsSlash.U3.Matrix`), which happens over Jacobs' own coefficient field `K₃ = ℚ₃`
...
(`PhD.JacobsSlash.U3.HeckeSlopes`).  But a reader should not conclude from this file that the
result needs an infinite-dimensional coefficient field.

## The cyclotomic certificate (not here — see `PhD.JacobsSlash.U3.HeckeSlopes`)
...
constructions.  This recipe is carried out in `PhD.JacobsSlash.U3.HeckeSlopes`, where the
