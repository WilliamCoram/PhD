...
* `QMF.U3.γ₉ : ℤᵐ⁰` — the wild-level threshold `v(9)`, and `Sigma1`, the submonoid of
  ...
  which is what makes the weight action of `PhD.JacobsSlash.U3Data` apply.
...
`PhD.JacobsSlash.{PadicAnalytic, U3Data, Slopes}` be instantiated at `K = K₃`.
...
(`PhD/ForMathlib/NumberTheory/NumberField/Completion/FinitePlace.lean`, mathlib's scoped
...
`hνc` of `PhD.JacobsSlash.U3Data` at `K₃`.  (The proof gives `‖3‖¹¹`, one power stronger:
...
The `κ`-analytics of `PhD.JacobsSlash.U3.«4_KappaColumn»`/`«5_KappaWeight»` only ever use the mod-`3`
