...
def ends_at (segs : List (Segment Γ)) (j₀ : ℕ) (j₁ : Γ) : Prop :=
  ...
  | some s => ∃ l : ℕ, s.l = l ∧ s.x + l = j₀ ∧ v j₀ = j₁
  | none => True
...
lemma connected_append_singleton {segs : List (Segment Γ)} (seg : Segment Γ)
  ...
  induction segs with
  | nil =>
      simp [NewtonPolygonData.connected]
  | cons s ss ih =>
      cases ss with
      | nil =>
          simp [ends_at] at h_end
          rcases h_end with ⟨l, hl, hx, hy⟩
          change ∃ l' : ℕ, s.l = l' ∧ seg.x = s.x + l' ∧ v (s.x + l') = seg.y ∧ True
          refine ⟨l, hl, hx.symm, ?_, trivial⟩
          simpa [hx] using hy
      | cons s' ss' =>
          simp [NewtonPolygonData.connected] at h_conn ⊢
          rcases h_conn with ⟨l, hl, hx, hy, htail⟩
          refine ⟨l, hl, hx, hy, ?_⟩
          apply ih htail
          simpa [ends_at]
  ...
  · unfold buildNewtonPolygon.build
    ...
    · simp only [Nat.add_eq_zero_iff, one_ne_zero, and_false, ↓reduceIte, add_tsub_cancel_right]
      ...
      · refine ih (segs := segs ++ [mkSegment i₀ i₁ (j₀ - i₀) m true])
        ...
        refine ⟨j₀ - i₀, rfl, Nat.add_sub_of_le hij.le, nextVertex_j₁_eq (Γ := Γ) v h⟩
      · simpa [getSegments] using h_conn
