...
variable {K : Type*} [CommSemiring K] [Algebra K ℝ]
...
def slopeSet (i₀ : ℕ) (i₁ : K) : Set ℝ :=
  {m | ∃ j₀ : ℕ, j₀ > i₀ ∧ finite v j₀ ∧ ∃ j₁ : K, v j₀ = j₁ ∧ m = slopeReal i₀ j₀ i₁ j₁}

def achievingSet (i₀ : ℕ) (i₁ : K) (m : ℝ) : Set ℕ :=
  {j : ℕ | j > i₀ ∧ finite v j ∧ ∃ j₁ : K, v j = j₁ ∧ m = slopeReal i₀ j i₁ j₁}
...
def Segment.supporting (seg : Segment K) : Prop :=
  ∀ j₀ > seg.i₀, finite v j₀ → ∀ (j₁ : K), j₁ = v j₀ →
  algebraMap K ℝ j₁ ≥ lineAt seg.i₀ seg.i₁ seg.slope j₀

/-- A segment is tight if its endpoint (j₀, j₁) lies exactly on the line. -/
def Segment.tight (seg : Segment K) : Prop :=
  algebraMap K ℝ seg.j₁ = lineAt seg.i₀ seg.i₁ seg.slope seg.j₀
...
def FinalRay.supporting (ray : FinalRay K) : Prop :=
  ∀ j > ray.i₀, finite v j → ∀ (j₁ : K), v j = j₁ →
  algebraMap K ℝ j₁ ≥ lineAt ray.i₀ ray.i₁ ray.slope j
...
-/
noncomputable def buildNewtonPolygon (n : ℕ) : BuildResult K :=
  open Classical in
...
noncomputable
def newtonPolygon (n : ℕ)  : NewtonPolygonData K :=
...
lemma getSegments_eq (n : ℕ) {i₀ : ℕ} {i₁ : K} (h : findFirstFinite v 0 = some (i₀, i₁)) :
...
lemma getResult_eq (n : ℕ) {i₀ : ℕ} {i₁ : K}
...
lemma nextVertex_bddBelow (v : ValSeq K) {i₀ j₀ : ℕ} {i₁ j₁ : K}
    (h :  nextStep v i₀ i₁ = StepResult.nextVertex j₀ j₁) :
    BddBelow (slopeSet v i₀ i₁) := by
  simp_rw [nextStep] at h
  grind

lemma nextVertex_finite (v : ValSeq K) {i₀ j₀ : ℕ} {i₁ j₁ : K}
    (h :  nextStep v i₀ i₁ = StepResult.nextVertex j₀ j₁) :
...
lemma nextVertex_nonEmpty (v : ValSeq K) {i₀ j₀ : ℕ} {i₁ j₁ : K}
    (h :  nextStep v i₀ i₁ = StepResult.nextVertex j₀ j₁) :
...
lemma nextVertex_j₀_eq_max (v : ValSeq K) {i₀ j₀ : ℕ} {i₁ j₁ : K}
...
lemma nextVertex_j₀Mem (v : ValSeq K) {i₀ j₀ : ℕ} {i₁ j₁ : K}
    (h :  nextStep v i₀ i₁ = StepResult.nextVertex j₀ j₁) :
...
lemma nextVertex_j₁_eq (v : ValSeq K) {i₀ j₀ : ℕ} {i₁ j₁ : K}
    (h :  nextStep v i₀ i₁ = StepResult.nextVertex j₀ j₁) : v j₀ = j₁ := by
  simp_rw [nextStep] at h
  aesop

lemma nextVertex_j₀Finite (v : ValSeq K) {i₀ j₀ : ℕ} {i₁ j₁ : K}
    (h :  nextStep v i₀ i₁ = StepResult.nextVertex j₀ j₁) : finite v j₀ := by
  have := nextVertex_j₁_eq v h
  simp_all [finite]

lemma nextVertex_slope_eq_sInf (v : ValSeq K) {i₀ j₀ : ℕ} {i₁ j₁ : K}
    (h : nextStep v i₀ i₁ = StepResult.nextVertex j₀ j₁) :
