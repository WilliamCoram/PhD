...
Stated over the same generality as `PhD.Main.TateFredholm.«05_Fredholm»` (Banach ultrametric
`NormedCommRing`, arbitrary decidable index); candidates for upstreaming into
`PhD/Main/TateFredholm/`.
...
`PhD/Main/TateFredholm/` does: `rowNorm` is a real `⨆`, so comparing the row sup of a
