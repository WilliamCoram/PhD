...
`M₂,₂` is the middle factor of `det(1 − T·U₃)` (`Jacobs.charPowerSeries_U3MatrixOp`,
`PhD.Jacobs.DiamondW`) and `U₃`'s matrix is identified in `PhD.Jacobs.U3.Matrix`
...
formalised — it needs [Jacobs, Lemma 2.9]; see `PhD.Jacobs.DiamondW`'s module header. -/
