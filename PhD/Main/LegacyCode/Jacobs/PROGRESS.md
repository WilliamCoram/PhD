...
> **LEGACY (moved to `PhD/Main/LegacyCode/` on 2026-08-21).**  Frozen history of the
> left-action original; do not develop or import from here.  The live replacement is
> the right-slash fork `PhD/Main/JacobsSlash/` (see its `PROGRESS.md`).  The status below
...
1. **Serre/Fredholm theory** (`PhD/Main/TateFredholm/`): `det(1 − Tu)` for compactoid
...
- `PhD/Main/TateFredholm/` — [Ser62] / [Jac §1.2]: `charCoeff`/`charPowerSeries`
...
- `PhD/Main/QMF/` — abstract automorphic machinery: `AutomorphicFunction`,
...
- `PhD/Main/NewtonPolygons/` — the Newton-polygon spec/construction the slope reading
