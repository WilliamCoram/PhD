...
* `TateFredholm.idx j i` — the exponent `(j, i)` as a multidegree in `Fin 2 →₀ ℕ` (`x`-degree
  `j` = row, `y`-degree `i` = column, matching the thesis's `∑ a_m^{(r)} x^m y^r` with
  rows `m`).
* `TateFredholm.ofCoeffs M` — the operator `c(ℕ, K) →L[K] c(ℕ, K)` with matrix `M` (bounded
  entries, columns vanishing at infinity), i.e. `e_i ↦ ∑_j M j i • e_j`.
* `TateFredholm.ofGenFun F` — the operator attached to a two-variable power series.
* `TateFredholm.diagRescale α β F` — the coefficientwise rescaling `F(αx, βy)`; the diagonal
  matrices `D(α)` of [Jacobs, Notation 1.2] act on generating functions through it
  ([Jacobs, Proposition 1.3]).
* `TateFredholm.diagOp a` — the diagonal operator `diag(a₀, a₁, …)` for `‖a n‖ ≤ 1`.
...
`TateFredholm.ext_matrixCoeff` in `PhD/Main/TateFredholm/04_Matrix.lean`. -/
...
/-- [Jacobs, Corollary 1.10] in compactoid form with a real decay rate: geometric row
decay `‖matrixCoeff u j i‖ ≤ C · σ^j` for some `0 ≤ σ < 1` makes `u` compactoid. -/
theorem isCompactoid_of_row_decay' {u : c(ℕ, K) →L[K] c(ℕ, K)} {σ C : ℝ} (hσ0 : 0 ≤ σ)
    (hσ : σ < 1) (h : ∀ j i, ‖matrixCoeff u j i‖ ≤ C * σ ^ j) : IsCompactoid u := by
  have hgeom : Tendsto (fun j : ℕ => C * σ ^ j) cofinite (𝓝 0) := by
    rw [Nat.cofinite_eq_atTop]
    simpa using (tendsto_pow_atTop_nhds_zero_of_lt_one hσ0 hσ).const_mul C
  have hle : ∀ j, rowNorm u j ≤ C * σ ^ j := fun j =>
    Real.iSup_le (fun i => h j i) ((norm_nonneg _).trans (h j 0))
  exact tendsto_of_tendsto_of_tendsto_of_le_of_le tendsto_const_nhds hgeom
    (fun j => rowNorm_nonneg u j) hle
