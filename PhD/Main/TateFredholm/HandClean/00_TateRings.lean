...
* **The normed one** (`TateFredholm.IsTate`, [JN] Definition 2.1.2): a *normed* ring carrying a
  ...
  all `x`.  This is the standing hypothesis of `PhD/Main/TateFredholm/`.
...
`PhD/Main/TateFredholm/` development, replacing `[NormedAlgebra K R]` of the field-based
blueprints. -/
...
So `IsTate` — the standing hypothesis of `PhD/Main/TateFredholm/` — is a special case of the general
