import PhD.Main.TateFredholm.«01_OperatorNorm»
...
`PhD/Main/TateFredholm/01_OperatorNorm.lean`.
-/
...
definitionally — so the PR would let `PhD/Main/TateFredholm/01_OperatorNorm.lean` drop its own
