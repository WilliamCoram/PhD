...
import PhD.Main.ForMathlib.Topology.Algebra.Bounded
import PhD.Main.ForMathlib.Topology.Algebra.PowerBounded
import PhD.Main.ForMathlib.Topology.Algebra.TopologicallyNilpotent
import PhD.Main.ForMathlib.Topology.Algebra.Nonarchimedean.LinearTopology
...
Ported from the `«Adic spaces»` project onto this repository's `ForMathlib` bounded /
power-bounded API (`PhD/Main/ForMathlib/Topology/Algebra/{Bounded, PowerBounded,
TopologicallyNilpotent}.lean`).  Three differences are absorbed here:

* `TopologicalRing.IsBounded S` is `∀ U ∈ 𝓝 0, ∃ V ∈ 𝓝 0, V * S ⊆ U` (multiplier on the left);
  `isBounded_of_forall_mul` and `IsBounded.exists_mul_subset` below convert to and from the
  `S * V ⊆ U` form.
* `PowerBounded.IsPowerBounded` / `powerBoundedSubring A` are `PowerBounded.IsPowerBounded` /
  `PowerBounded.subring A (S := ℤ)`, the `ℤ`-linear-topology instance coming from
  `IsHuberRing.nonarchimedeanAddGroup`.
* `topologicallyNilpotentElements` and `topNilpAddSubgroup` are supplied below; they are candidates
  for `PhD/Main/ForMathlib/Topology/Algebra/TopologicallyNilpotent.lean`.

One further repair, unrelated to the port: the `h_agree` step of
`IsTateRing.isAdicHom_of_continuous_with_pairs` used `congr 1; ring`, which split
`φ(u^K)^e₁ = φ(u)^e₂` into two false goals; it now normalises both sides to powers of `φ u`
first (`ha_coe`, `hvL_coe`).
-/
...
section IsBoundedComm

variable {M : Type*} [CommMonoidWithZero M] [TopologicalSpace M] {S : Set M}

/-- Boundedness stated with the multiplier on the right, as `S * V ⊆ U`. -/
theorem isBounded_of_forall_mul (h : ∀ U ∈ 𝓝 (0 : M), ∃ V ∈ 𝓝 (0 : M), S * V ⊆ U) :
    IsBounded S := fun U hU ↦
  let ⟨V, hV, hVU⟩ := h U hU
  ⟨V, hV, by rwa [mul_comm]⟩

/-- A bounded set absorbs neighbourhoods of `0` on the right too. -/
theorem IsBounded.exists_mul_subset (hS : IsBounded S) (U : Set M) (hU : U ∈ 𝓝 (0 : M)) :
    ∃ V ∈ 𝓝 (0 : M), S * V ⊆ U :=
  let ⟨V, hV, hVU⟩ := hS U hU
  ⟨V, hV, by rwa [mul_comm]⟩
...
/-- The set `R°°` of topologically nilpotent elements of `R`. -/
def topologicallyNilpotentElements : Set R := {a | IsTopologicallyNilpotent a}

variable [IsTopologicalRing R] [IsLinearTopology ℤ R]

/-- `R°°` as an additive subgroup.  Closure under addition is exactly the nonarchimedean
hypothesis, here in the form `IsLinearTopology ℤ R`. -/
def topNilpAddSubgroup : AddSubgroup R where
  carrier := topologicallyNilpotentElements R
  add_mem' := IsTopologicallyNilpotent.add' ℤ
  zero_mem' := IsTopologicallyNilpotent.zero
  neg_mem' {a} ha := by
    rw [← neg_one_mul]
    exact IsTopologicallyNilpotent.mul_left_of_isPowerBounded
      (isPowerBounded_neg isPowerBounded_one) ha

@[simp] theorem mem_topNilpAddSubgroup {a : R} :
    a ∈ topNilpAddSubgroup R ↔ IsTopologicallyNilpotent a := Iff.rfl

@[simp] theorem coe_topNilpAddSubgroup :
    (topNilpAddSubgroup R : Set R) = topologicallyNilpotentElements R := rfl
...
theorem isBounded_A₀ (P : PairOfDefinition A) :
  ...
  TopologicalRing.isBounded_of_forall_mul P.isBounded_subring
...
rings. -/
theorem isOpen_powerBoundedSubring [IsLinearTopology ℤ A] (P : PairOfDefinition A) :
    IsOpen ((PowerBounded.subring A (S := ℤ) : Subring A) : Set A) := by
  have : NonarchimedeanAddGroup A := P.nonarchimedeanAddGroup
  have h_le : P.A₀.toAddSubgroup ≤
      (PowerBounded.subring A (S := ℤ)).toAddSubgroup :=
...
theorem IsTateRing.isOpen_topologicalNilradical :
  ...
  have hsub : (u : A) • (P.A₀ : Set A) ⊆
    ...
    exact IsTopologicallyNilpotent.mem_topologicalNilradical_iff.mpr
      (IsTopologicallyNilpotent.mul_left_of_isPowerBounded (P.mem_powerBoundedSubring ha) hu)
...
theorem IsTateRing.isOpen_topologicallyNilpotentElements_nonarch :
  ...
  have hsub : (u : A) • (P.A₀ : Set A) ⊆
    ...
    exact IsTopologicallyNilpotent.mul_left_of_isPowerBounded (P.mem_powerBoundedSubring ha) hu
  exact AddSubgroup.isOpen_of_mem_nhds _ (Filter.mem_of_superset
...
private theorem isBounded_addSubgroup_closure [NonarchimedeanRing A]
  ...
  refine TopologicalRing.isBounded_of_forall_mul fun U hU ↦ ?_
  obtain ⟨G, hGU⟩ := NonarchimedeanRing.is_nonarchimedean U hU
  obtain ⟨V, hV, hSV⟩ := hS.exists_mul_subset (G : Set A) (G.isOpen.mem_nhds G.zero_mem)
...
theorem PairOfDefinition.isBounded_adjoin (P : PairOfDefinition A) (T : Finset A)
    (hT : ∀ t ∈ T, PowerBounded.IsPowerBounded t)
  ...
  | @insert a S ha ih =>
    have hT_S : ∀ t ∈ S, PowerBounded.IsPowerBounded t :=
      fun t ht ↦ hT t (Finset.mem_insert_of_mem ht)
    specialize ih hT_S
    set B_old := Subring.closure ((P.A₀ : Set A) ∪ ↑S)
    have ha_pb : PowerBounded.IsPowerBounded a := hT a (Finset.mem_insert_self a S)
...
def PairOfDefinition.adjoin (P : PairOfDefinition A) (T : Finset A)
  ...
  isAdic := by
    ...
    · intro s hs
      ...
      obtain ⟨V, hV, hBV⟩ := (P.isBounded_adjoin T hT).exists_mul_subset (G : Set A)
...
theorem IsTateRing.isAdicHom_of_continuous_with_pairs [IsTateRing A] [IsHuberRing B]
  ...
  have ha_coe : ((a : PB.A₀) : B) = φ (u : A) ^ (K * N) := by
    show φ ((uK ^ N : PA.A₀) : A) = _
    rw [SubmonoidClass.coe_pow, show ((uK : PA.A₀) : A) = (u : A) ^ K from rfl, ← pow_mul, map_pow]
  have hvL_coe : ((vL : PB.A₀) : B) = φ (u : A) ^ L := by
    show ((Units.map (φ : A →* B) u : Bˣ) : B) ^ L = _
    rw [Units.coe_map]
    rfl
  have h_agree : a ^ (L * M) = (vL ^ M) ^ (K * N) :=
    Subtype.ext (by
      rw [SubmonoidClass.coe_pow, SubmonoidClass.coe_pow, SubmonoidClass.coe_pow,
        ha_coe, hvL_coe, ← pow_mul, ← pow_mul, ← pow_mul]
      ring)
