...
A first draft of the equivariance (in `PhD/Main/LWX/11_Theta.lean`, removed on 2026-09-09) let the two
...
`private` in `PhD/Main/QMF/Weight/06_Algebraic.lean`; reproved here rather than de-privatising it, which
...
proper home is `PhD/Main/LWX/09_DiscModel.lean`; stated here to avoid rebuilding that tree.) -/
...
`PhD/Main/TateFredholm/08_BlockMap.lean`; kept here to avoid rebuilding that tree.) -/
