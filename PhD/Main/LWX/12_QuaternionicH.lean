...
-/
import PhD.Main.LWX.«11_SeamH»
import PhD.Main.LWX.«08_Quaternionic»
...
`m = 1` seam (`PhD/Main/LWX/08_Quaternionic.lean`), the disc model is built directly on the **integral**
component map `thetaInt : (D ⊗ 𝔸_f)ˣ →* M₂(ℚ_p)` — the disc action of `PhD/Main/LWX/09_DiscModel.lean`
