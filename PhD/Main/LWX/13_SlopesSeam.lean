...
`PhD/Main/LWX/{Halo,Sharpness,UpperPolygon,Vertices,Claim,SlopeRatios}.lean` prove [LWX]'s halo
...
**integral matrix** of a certificate datum.  `PhD/Main/LWX/11_SeamH.lean` identifies that series with
...
`ψ = LWX.padicComparison p` (as in `PhD/Main/LWX/12_QuaternionicH.lean`); no separate statement is
given for it here.
...
`PhD/Main/LWX/08_SlopeGrowth.lean`; they apply verbatim to the ratios appearing below.
