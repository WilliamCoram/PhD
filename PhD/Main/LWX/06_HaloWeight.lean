...
-/
import PhD.Main.LWX.«01_Binomial»
import PhD.Main.LWX.«05_Specialize»
...
`s = log(1+T₀)/p`.  The general-weight layer `PhD/Main/QMF/Weight/` needs a weight as an
...
* the column is `(cz+d)²·κ_{T₀}(d)·∑_m C(s, m)(c/d)^m z^m` — `κ_{T₀}(1 + wz) = (1 + wz)^s`
  expanded by the binomial theorem `PhD/Main/LWX/01_Binomial.lean` — with decay
  ...
  `m = 1` sub-annulus is what the Tate-algebra model of `PhD/Main/QMF/Weight/` can express.)
