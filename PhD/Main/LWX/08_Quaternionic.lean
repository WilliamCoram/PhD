...
import PhD.Main.ForMathlib.NumberTheory.Padics.AdicCompletionEquiv
...
The general-weight layer's quaternionic instantiation (`PhD/Main/QMF/Weight/07_Quaternionic.lean`)
works over a number field `F` at a place `v` with `K = F_v = v.adicCompletion F`; the integral
model of `PhD/Main/LWX/04_IntegralModel.lean` needs `ℤ_p`-entries, i.e. `θ` valued in `M₂(ℚ_p)`.  At
`F = ℚ`, `v = (p)`, mathlib's comparison `Padic.adicCompletionEquiv : ℚ_[p] ≃A[ℚ] ℚ_v` (an
isometry, `PhD/Main/ForMathlib/…/AdicCompletionEquiv.lean`) transports one to the other: with
`ψ = E : ℚ_p → K_p` and `θ = E⁻¹ ∘ toMatrix ℚ D v`, the `K`-side map `thetaK ψ θ` of
`PhD/Main/LWX/07_Seam.lean` is literally `toMatrix ℚ D v`, so `IntForms θ … U` is [LWX]'s `S^D_int` at
