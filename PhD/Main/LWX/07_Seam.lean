...
-/
import PhD.Main.LWX.«06_HaloWeight»
import PhD.Main.LWX.«00_Colmez»
import PhD.Main.LWX.«05_Certificates»
...
halo weight is `1`-analytic, `PhD/Main/LWX/06_HaloWeight.lean`), identifying
`specCharSeries D ω ψ T₀` (`Char(P)` evaluated at `T₀`, `PhD/Main/LWX/04_Halo.lean`) with the
...
3. `monomialToMahler = diag(m!) ∘ colmezEquiv.symm` (`PhD/Main/LWX/00_Colmez.lean`), so the Colmez-basis
