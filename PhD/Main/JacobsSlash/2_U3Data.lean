...
-/
import PhD.Main.JacobsSlash.«1_PadicAnalytic»
import PhD.Main.TateFredholm.«05_GenFun»
import PhD.Main.TateFredholm.«06_WeightGenFun»
...
## The identification (PROVED — `PhD.Main.JacobsSlash.U3.Matrix`)
...
* Theorem 2.1 (`D^×_f = ∐ D^× cᵢ U₁(9)`) — `JacobsSlash.U3.classRep_complete`, and
  Lemma 2.2 (`Γᵢ = 1`) — `JacobsSlash.U3.stabilizerAt_classRep`, both in `PhD.Main.JacobsSlash.U3.ClassSet`;
* Lemma 2.3 (the decomposition of `Uη₃U`) — `JacobsSlash.U3.bijOn_etaRep`
  (`PhD.Main.JacobsSlash.U3.EtaDecomposition`);
* Lemmas 2.4/2.5 and §B.1 (the nine factorisations `cᵢv_t⁻¹ = d·c(i,t)·u(i,t)`) —
  ...
  (`PhD.Main.JacobsSlash.U3.Factorisations`), *recomputed* by exhaustive search
  (`PhD/Jacobs/U3/certificate_search.py`), not transcribed;
* Proposition 2.6 (the generating function of the weight-`κ` action) —
  `JacobsSlash.matrixCoeff_kappaSlash_jacobsWeight` (`PhD.Main.JacobsSlash.U3.«5_KappaWeight»`), whose composition law
  needed the analytic-substitution development of `PhD.Main.JacobsSlash.U3.Compose` and
  `PhD.Main.JacobsSlash.BinomialTheorem`;
* the identification itself — `JacobsSlash.U3.heckeU3_apply_classRep` (unconditional) and
  ...
  `PhD.Main.JacobsSlash.U3.Matrix`.
...
(`PhD.Main.JacobsSlash.U3.«7_Fredholm»` — twist-free in this fork's right-slash
orientation), and the eigenblock factorisation
`det(1 − T·U₃) = ∏ₜ det(1 − T·M_{t,t})` holds over every isometric extension
containing `ω`, witnessed at `L₃ = ℚ₃(ζ₃)` (`PhD.Main.JacobsSlash.U3.«8_HeckeSlopes»`).
There are no external sorries: `hClassNumberOne` ([Jacobs, Lemma 1.22]) is proven in
`PhD/Main/JacobsSlash/CN1/` (Voight's Euclidean → principal → idelic-dictionary route);
...
(p. 28); their derivation (Lemmas 2.3–2.5, §B.1) is proved in `PhD.Main.JacobsSlash.U3.Factorisations`
...
function) now live in `PhD/Main/TateFredholm/06_WeightGenFun.lean` (namespace `TateFredholm`),
...
is formalised in `JacobsSlash.matrixCoeff_kappaSlash_jacobsWeight` (`PhD.Main.JacobsSlash.U3.«5_KappaWeight»`), which
proves this series really is the generating function of `‖_κ γ` acting on the Tate
algebra, and `JacobsSlash.U3.sum_weightGenFun_eq_h` (`PhD.Main.JacobsSlash.U3.Factorisations`) assembles
...
/- `fin2_eq_zero` moved to `PhD/Main/TateFredholm/06_WeightGenFun.lean`. -/
...
/- `linSeries_eq` / `quadSeries_eq` moved to `PhD/Main/TateFredholm/06_WeightGenFun.lean`. -/
...
`PhD/Main/TateFredholm/06_WeightGenFun.lean`. -/
...
(`PhD.Main.JacobsSlash.DiamondW`) for the factorisation and `JacobsSlash.U3.heckeU3_apply_classRep`
(`PhD.Main.JacobsSlash.U3.Matrix`) for the identification of `U₃`.  The *eigenspace* reading is
not formalised (it needs [Jacobs, Lemma 2.9]; see `PhD.Main.JacobsSlash.DiamondW`'s header). -/
