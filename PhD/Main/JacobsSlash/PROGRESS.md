...
original (moved to `PhD/Main/LegacyCode/Jacobs/` on 2026-08-21: NEVER import it).
Allowed imports: `Mathlib`, `PhD.Main.TateFredholm`, `PhD.Main.QMF` (including the
right-slash layer `PhD.Main.QMF.Slash`), `PhD.Main.NewtonPolygons`, and earlier
`PhD.Main.JacobsSlash` files.  `import PhD.Main.LegacyCode.Jacobs` (or the defunct
...
* Right slash throughout: `RightSlashAction` / `f ∣ₛ δ` (`PhD.Main.QMF.Slash.«02_Basic»`),
...
2. `heckeU3_apply_classRep` — **unconditional**: `(U₃ φ)(cᵢ) = Σⱼ blockEntry i j (φ(cⱼ))` —
   ...
   `PhD/Main/QMF/Weight/06_Compact.lean` model instantiated — `evalU3 := evalAtReps … classRep`,
...
| file | role |
|---|---|
| `1_PadicAnalytic` | `p = 3` instance of the general `PhD/Main/LWX/00_PadicExpLog.lean` (exp/log re-exported + 8 `‖3‖ < 1` shims) plus the `p = 3`-specific `unitPow`/binomial-series sections; deduplicated 2026-09-05 (board `.mathlib-quality/jacobs-explog/`) |
| `1_GenFun` | generating-function calculus, `idx`, `ext_matrixCoeff` norm form |
| ~~`1_BlockOp`~~ → `PhD/Main/TateFredholm/06_BlockOp.lean` (moved 2026-08-18, namespace `TateFredholm`; forms-headline board R0) | `cSpace` blocks, `blockOp`, `matrixCoeff_*`, compactoid closure |
...
| `4_KappaColumn` (was `4_KappaSlash`, slimmed 2026-08-19, forms-headline T002) | the `Σ₁(3)` norm bounds and the κ-column closed form `coeff_yCoeff_kappaSeries₂`/`kappaSeries₂_eq_yExtend`; the weight *action* now lives in `PhD/Main/QMF/Weight/` |
...
* `1_SlopeTheorem.lean` no longer carries its own Hadamard/minor/combinatorics lemmas: the
  σ-general versions live in `PhD/Main/TateFredholm/06_Slopes.lean` (`norm_det_le_pow_of_row_bound`,
...
* `4_SlopeReading.lean` keeps only the two Jacobs cases; `NewtonPolygon₀.ofSlopes` and the four
  spec-level lemmas moved to `PhD/Main/NewtonPolygons/OfSlopes.lean`.
...
`PhD/Main/TateFredholm/06_Slopes.lean`; the fork keeps `ϖ₃`/`coe_ϖ₃` and applies the general theorems at
...
* `PhD/Main/QMF/Slash/` — the general right-slash abstract layer this fork consumes
...
* `PhD/Main/LegacyCode/Jacobs/` — the left-action original: moved to legacy by the
