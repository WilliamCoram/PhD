...
import PhD.Main.JacobsSlash.«1_SlopeTheorem»
...
and `ω` are documented where they are introduced, in `PhD.Main.JacobsSlash.U3Data`'s module
...
* the **identification** — the transcribed matrix is the matrix of `U₃`
  (`JacobsSlash.U3.heckeU3_apply_classRep`, `JacobsSlash.U3.charPowerSeries_blockOp_eq_U3MatrixOp`
  in `PhD.Main.JacobsSlash.U3.Matrix`; see `PhD.Main.JacobsSlash.U3Data`'s module header for the full list of
  formalised inputs, Thm 2.1 – Prop 2.6);
* the **factorisation** — `M₂,₂` is the middle factor of
  `det(1 − T·U₃) = det(1 − T·M₁,₁)·det(1 − T·M₂,₂)·det(1 − T·M₃,₃)`
  (`JacobsSlash.charPowerSeries_U3MatrixOp` in `PhD.Main.JacobsSlash.DiamondW`, via [Jacobs, Lemma 2.10]
...
`PhD.Main.JacobsSlash.DiamondW`'s module header).  Nothing downstream depends on it: the
block-diagonalisation `lemma210` is proved directly from the Lemma 2.11 identities.

## Slope reading (proved in `PhD.Main.JacobsSlash.SlopeReading`)
...
vertices ⇒ slopes = successive differences) is `JacobsSlash.unitSlope_newtonPolygon₀OfPowerSeries_M22op`
(the AG-NP tranche, `PhD.Main.JacobsSlash.SlopeReading`), which is where [Jacobs, Corollary 2.16]
is stated in full.
-/
...
`p 0 + p 1` through the recurrence `MvPowerSeries.coeff_inv` (compare `JacobsSlash.rowInt_inv`). -/
...
theorem norm_det_NgenFun_minor (n : ℕ) :
  ...
  have hAbd : ∀ j i : Fin n, ‖A j i‖ ≤ 1 := fun j i =>
    norm_coeff_NgenFun_le h3 ht hνc hω _
...
[Jacobs, Proposition 1.3] (`TateFredholm.matrixCoeff_diagOp_comp` is the two-sided version).
-/
...
private theorem norm_charCoeff_M22scaled
  ...
  · rw [hmc, norm_mul, norm_pow]
    calc ‖(3 : K)‖ ^ j * ‖coeff (idx j i) (NgenFun t ν ω)‖
        ≤ ‖(3 : K)‖ ^ j * 1 :=
          mul_le_mul_of_nonneg_left (norm_coeff_NgenFun_le h3 ht hνc hω _) (by positivity)
      _ = ‖(3 : K)‖ ^ j := mul_one _
  · have hmat : (Matrix.of fun j i : Fin n => (3 : K)⁻¹ ^ (j : ℕ) * matrixCoeff u j i)
      ...
      rfl
    rw [hmat]
    exact norm_det_NgenFun_minor h3 ht hνc hω n
...
valuation but does not generate the value group.  See the `PhD.Main.JacobsSlash.U3Data` module header. -/
