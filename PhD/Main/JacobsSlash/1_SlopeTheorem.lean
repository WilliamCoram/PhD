...
`PhD.Main.TateFredholm.«06_Slopes»` (`TateFredholm.norm_charCoeff_of_unit_minors`,
...
`v₃(3) = 1`.  The slope readings of `PhD.Main.JacobsSlash.«4_SlopeReading»` and the applications in
`PhD.Main.JacobsSlash.«3_Slopes»` consume the general theorems at `ϖ₃`.

`3` need not be a *uniformizer* of `K`, and under `hω : ω ^ 2 + ω + 1 = 0` (case B) it is not —
`K` is then ramified over `ℚ₃` with `v₃(K^×) = (1/2)ℤ`; see the `PhD.Main.JacobsSlash.U3Data` module
...
-- in `PhD/Main/TateFredholm/09_Riesz.lean`, so hoisting this one means resolving that name collision —
