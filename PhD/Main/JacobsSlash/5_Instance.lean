...
The case-B development (`PhD.Main.JacobsSlash.Slopes`, `PhD.Main.JacobsSlash.SlopeReading`) is stated over an
...
* **The half-integer slopes are forced there, not here.**  Over `ℚ₃(ζ₃)` the extension is
  ...
  [Jacobs, Cor 2.16] are *compelled* to be half-integers (see the `PhD.Main.JacobsSlash.U3Data` module
...
ones, and they are what compose with the (now proved) identification of `U₃`
(`PhD.Main.JacobsSlash.U3.Matrix`), which happens over Jacobs' own coefficient field `K₃ = ℚ₃`
rather than over `ℂ₃`.  Since `ℚ₃` contains no primitive cube root of unity, that
composition is stated over an extension `L ⊇ ℚ₃(ζ₃)` carrying an isometric `ℚ₃ →+* L`
(`PhD.Main.JacobsSlash.U3.HeckeSlopes`).  But a reader should not conclude from this file that the
result needs an infinite-dimensional coefficient field.

## The cyclotomic certificate (not here — see `PhD.Main.JacobsSlash.U3.HeckeSlopes`)

A second witness at the minimal field `ℚ₃(ζ₃)` certifies exactly the two facts above.  It is
plumbing rather than missing theory: mathlib has no *instance* `NormedField L` for a finite
extension `L/ℚ₃` (there cannot be one — it would be a diamond), but it does have the
constructions.  This recipe is carried out in `PhD.Main.JacobsSlash.U3.HeckeSlopes`, where the
extension is needed for a second reason — the identification of `U₃` lives over
`K₃ = ℚ₃`, which contains no `ω`, so the eigenblock factorisation is stated over an
extension carrying an isometric `K₃ →+* L`.  The recipe:
