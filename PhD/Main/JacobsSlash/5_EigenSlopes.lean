...
`PhD/Main/TateFredholm/10_NewtonSlopes.lean`, where `PhD/Main/LWX/15_StepThree.lean` also consumes it.
...
(The height-level transport of `PhD.Main.JacobsSlash.«4_SlopeReading»` pins `unitSlope`;
...
The `(ϖ₃ h3).val`-normalised slope data of `PhD.Main.JacobsSlash.«4_SlopeReading»` is
...
`PhD.Main.JacobsSlash.«4_DiamondW»`, as an eigenvector of the assembled `U₃`-matrix lying in
