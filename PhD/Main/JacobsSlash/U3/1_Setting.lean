...
-/
import PhD.Main.QMF.«03_Quaternionic»
...
* `QMF.U3.K₃ := v₃.adicCompletion ℚ` with its mathlib `NormedField` structure, and the
  ...
  `PhD.Main.JacobsSlash.U3Data` at `K₃`;
* `QMF.U3.ν₃ : K₃` — the Hensel square root of `−2` located at `2695 mod 3¹⁰`
  ...
  `JacobsSlash.Instance.exists_sqrt_neg_two_near`, which transports verbatim);
...
* `QMF.U3.γ₉ : ℤᵐ⁰` — the wild-level threshold `v(9)`, and `Sigma1`, the submonoid of
  `Sigma0' K₃ γ₉` of matrices whose `(1,1)`-entry is `≡ 1 mod 9` (the thesis's own form:
  ...
  which is what makes the weight action of `PhD.Main.JacobsSlash.U3Data` apply.

## Main results

* `JacobsSlash.theta_tmul_one`: the explicit matrix of `θ₃` on a quaternion, read off the
  four coordinates against `1, i, j, k` — the computational heart of every local check.
* `JacobsSlash.sq_ν₃`, `JacobsSlash.ν₃_near`: `ν₃² = −2` and its `3`-adic location
  `ν₃ ≡ 2695 mod 3¹⁰`, the two facts every entry-valuation downstream rests on.
* `JacobsSlash.valued_three_eq`, `JacobsSlash.valued_nine_eq`: `3` is a uniformizer at `v₃`
  and `v(9) = γ₉`, so the mod-`9` threshold really is reached by `9`.
* `JacobsSlash.sigma1_of_mul_eq_one`: `Σ₁(9)` is closed under inversion *inside* the
  integral matrices — read off `MN = NM = 1` without ever forming `M⁻¹`.
* `JacobsSlash.valuation_three_eq_one_of_ne`,
  `JacobsSlash.inv_three_mem_adicCompletionIntegers`: away from `3`, both `3` and `3⁻¹` are
...
`PhD.Main.JacobsSlash.{PadicAnalytic, U3Data, Slopes}` be instantiated at `K = K₃`.
...
structure, and
`NontriviallyNormedField` is the project's global instance on adic completions
(`ForMathlib/…/Completion/FinitePlace.lean`: mathlib's `Valued.toNontriviallyNormedField`
on top of mathlib's `NormedField` instance — same norm, no diamond). -/
...
/- `NontriviallyNormedField K₃` is the general instance on adic completions
(`PhD/Main/ForMathlib/NumberTheory/NumberField/Completion/FinitePlace.lean`, mathlib's scoped
`Valued.toNontriviallyNormedField` registered globally), found through `K₃ = v₃.adicCompletion ℚ`. -/
noncomputable example : NontriviallyNormedField K₃ := inferInstance
...
private theorem norm_ν₃_sub_one_le : ‖ν₃ - 1‖ ≤ ‖(3 : K₃)‖ := by
  ...
  have h2 : ‖(2 : K₃)‖ = 1 :=
    JacobsSlash.norm_ofNat_eq_one h3 (n := 2) (by norm_num) (by norm_num)
  ...
  have hm : ‖x - 1‖ ≤ 1 := by
    refine (JacobsSlash.norm_sub_le_max' x 1).trans ?_
    simp [hnx]
  have hp : ‖x + 1‖ ≤ 1 := by
  ...
  have hmax : (1 : ℝ) ≤ max ‖x + 1‖ ‖x - 1‖ := by
    calc (1:ℝ) = ‖(x + 1) - (x - 1)‖ := by
      ...
      _ ≤ max ‖x + 1‖ ‖x - 1‖ := JacobsSlash.norm_sub_le_max' _ _
  unfold ν₃
...
`hνc` of `PhD.Main.JacobsSlash.U3Data` at `K₃`.  (The proof gives `‖3‖¹¹`, one power stronger:
`(ν₃−2695)(ν₃+2695) = −3¹¹·41` with the second factor a unit since
`ν₃ + 2695 ≡ 1 + 2695 ≡ 2 mod 3`.) -/
theorem ν₃_near : ‖ν₃ - 2695‖ ≤ ‖(3 : K₃)‖ ^ 10 := by
  have h3 : ‖(3 : K₃)‖ < 1 := norm_three_lt_one
  have h41 : ‖(41 : K₃)‖ = 1 :=
    JacobsSlash.norm_ofNat_eq_one h3 (n := 41) (by norm_num) (by norm_num)
  have h2696 : ‖(2696 : K₃)‖ = 1 :=
    JacobsSlash.norm_ofNat_eq_one h3 (n := 2696) (by norm_num) (by norm_num)
  ...
  have hplus : ‖ν₃ + 2695‖ = 1 := by
    ...
    rw [JacobsSlash.norm_eq_of_sub_lt hsub, h2696]
...
theorem norm_two_eq_one : ‖(2 : K₃)‖ = 1 :=
  JacobsSlash.norm_ofNat_eq_one norm_three_lt_one (n := 2) (by norm_num) (by norm_num)
...
def qi : D := ⟨0, 1, 0, 0⟩
/-- The quaternion unit `j` of `D`, componentwise. -/
def qj : D := ⟨0, 0, 1, 0⟩

/-- The quaternion unit `k` of `D`, componentwise. -/
def qk : D := ⟨0, 0, 0, 1⟩
...
/-- The splitting `θ : D ⊗[ℚ] K₃ ≅ M₂(K₃)` of the quaternion algebra at `3`, built from
a square root `ν` of `−2` in `K₃` ([Jacobs p. 19]'s identification of `D₃` with
`M₂(ℚ₃)`). -/
...
/-- `Σ₁(9) ⊆ Σ₀'(v(9))`: the submonoid of matrices with `(1,1)`-entry `≡ 1 mod 9`
(valuatively: `v(g₁₁ − 1) ≤ v(9)`) — Jacobs's monoid of matrices with `c ≡ 0`,
`d ≡ 1 mod 9`, stated in the thesis's own (right-handed) convention.  The natural
domain of the weight-`κ` slash, since `g₁₁` is then a `1`-unit and `κ(g₁₁) = g₁₁ᵗ`
needs no Teichmüller factor. -/
def Sigma1 : Submonoid (Matrix (Fin 2) (Fin 2) K₃) where
  ...
  mul_mem' := by
    ...
    have hexp : (a * b) 1 1 - 1
        = (a 1 1 - 1) * b 1 1 + (b 1 1 - 1) + a 1 0 * b 0 1 := by
    ...
    refine le_trans (Valued.v.map_add _ _) (max_le (le_trans (Valued.v.map_add _ _)
      (max_le ?_ ?_)) ?_)
    · rw [map_mul]
      calc Valued.v (a 1 1 - 1) * Valued.v (b 1 1)
          ≤ γ₉ * 1 := mul_le_mul' ha.2 (Sigma0'.entry_le_one ⟨b, hb.1⟩ 1 1)
        _ = γ₉ := mul_one _
    · exact hb.2
    · rw [map_mul]
      calc Valued.v (a 1 0) * Valued.v (b 0 1)
          ≤ γ₉ * 1 := mul_le_mul' (Sigma0'.v_c_le ⟨a, ha.1⟩)
            (Sigma0'.entry_le_one ⟨b, hb.1⟩ 0 1)
        _ = γ₉ := mul_one _
...
Mirror of the left library's argument at the `(1,1)`-slot: `M₁₁N₁₀ = −M₁₀N₀₀` gives the
lower-left congruence (since `v(M₁₁) = 1`); `M₁₀N₀₁ + M₁₁N₁₁ = 1` forces `v(N₁₁) = 1`;
and `N₁₁ − 1 = N₁₁(1−M₁₁) − M₁₀N₀₁` is `≤ γ₉` because both summands are. -/
theorem sigma1_of_mul_eq_one {M N : Matrix (Fin 2) (Fin 2) K₃} (hM : M ∈ Sigma1)
    (hN : ∀ i j, Valued.v (N i j) ≤ 1) (h1 : M * N = 1) (_h2 : N * M = 1) :
    N ∈ Sigma1 := by
  obtain ⟨⟨hMint, hMc, hMd, hMdet⟩, hMd1⟩ := hM
  have e1 : M 1 0 * N 0 1 + M 1 1 * N 1 1 = 1 := by
    have := congrFun (congrFun h1 1) 1
    rwa [Matrix.mul_apply, Fin.sum_univ_two, Matrix.one_apply_eq] at this
  have e2 : M 1 0 * N 0 0 + M 1 1 * N 1 0 = 0 := by
    have := congrFun (congrFun h1 1) 0
    rwa [Matrix.mul_apply, Fin.sum_univ_two,
      Matrix.one_apply_ne (by decide : (1 : Fin 2) ≠ 0)] at this
  -- lower-left: `M₁₁ N₁₀ = −M₁₀ N₀₀`
  have hNc : Valued.v (N 1 0) ≤ γ₉ := by
    have hkey : M 1 1 * N 1 0 = -(M 1 0 * N 0 0) := by linear_combination e2
    have hv : Valued.v (N 1 0) = Valued.v (M 1 0 * N 0 0) := by
      have := congrArg Valued.v hkey
      rwa [map_mul, hMd, one_mul, Valuation.map_neg] at this
    rw [hv, map_mul]
    exact le_trans (mul_le_mul' hMc (hN 0 0)) (by rw [mul_one])
  -- `v(N₁₁) = 1`
  have hbn : Valued.v (M 1 0 * N 0 1) < 1 := by
    rw [map_mul]
    exact lt_of_le_of_lt (le_trans (mul_le_mul' hMc (hN 0 1)) (by rw [mul_one]))
      γ₉_lt_one
  have hNd : Valued.v (N 1 1) = 1 := by
    have h : Valued.v (M 1 1 * N 1 1) = 1 := by
      have hsum : M 1 1 * N 1 1 = 1 + -(M 1 0 * N 0 1) := by linear_combination e1
      have hlt2 : Valued.v (-(M 1 0 * N 0 1)) < Valued.v (1 : K₃) := by
        rwa [Valuation.map_neg, map_one]
      rw [hsum, Valued.v.map_add_eq_of_lt_left hlt2, map_one]
    rwa [map_mul, hMd, one_mul] at h
  refine ⟨⟨hN, hNc, hNd, ?_⟩, ?_⟩
  · intro hdet
    have := congrArg Matrix.det h1
    rw [Matrix.det_mul, hdet, mul_zero, Matrix.det_one] at this
    exact one_ne_zero this.symm
  · have hsub : N 1 1 - 1 = N 1 1 * (1 - M 1 1) + -(M 1 0 * N 0 1) := by
      linear_combination e1
    rw [hsub]
    refine le_trans (Valued.v.map_add _ _) (max_le ?_ ?_)
    · rw [map_mul]
      refine le_trans (mul_le_mul' (le_of_eq hNd) ?_) (by rw [one_mul])
      rw [show (1 : K₃) - M 1 1 = -(M 1 1 - 1) by ring, Valuation.map_neg]
      exact hMd1
    · rw [Valuation.map_neg, map_mul]
      exact le_trans (mul_le_mul' hMc (hN 0 1)) (by rw [mul_one])
...
theorem mem_sigma1_iff {g : Matrix (Fin 2) (Fin 2) K₃} :
    g ∈ Sigma1 ↔ g ∈ Sigma0' K₃ γ₉ γ₉_lt_one ∧
      Valued.v (g 1 1 - 1) ≤ γ₉ := Iff.rfl

/-! ### The tame acting monoid `Σ₁(3)`

The `κ`-analytics of `PhD.Main.JacobsSlash.U3.«4_KappaColumn»`/`«5_KappaWeight»` only ever use the mod-`3`
threshold (every `γ₉`-hypothesis there is immediately weakened through `γ₉ ≤ v(3)`), and
the `μ`-acting elements of the diamond operator are `1`-units only mod `3`.  So `Σ₁(3)` —
not `Σ₁(9)` — is the honest domain of the weight-`κ` slash, and it is the single acting
monoid of the whole development. `Σ₁(9)` survives as the *level* congruence cutting out
`U₁(9)`. -/

/-- The tame threshold `v(3)`: elements of value group `ℤᵐ⁰` at exponent `−1`. -/
def γ₃ : WithZero (Multiplicative ℤ) := (Multiplicative.ofAdd (-1 : ℤ) : Multiplicative ℤ)

theorem γ₃_lt_one : γ₃ < 1 := by decide

/-- The thresholds compare as the moduli do: `v(9) ≤ v(3)`. -/
theorem γ₉_le_γ₃ : γ₉ ≤ γ₃ := by decide

/-- `v(3) = γ₃` exactly: the mod-`3` threshold *is* the valuation of `3`. -/
theorem valued_three_eq_γ₃ : Valued.v (3 : K₃) = γ₃ := by
  rw [valued_three_eq, γ₃, WithZero.exp_eq_coe_ofAdd]
...
(which consume `‖d − 1‖ ≤ ‖3‖`, never the mod-`9` congruence).  Contains `Σ₁(9)`
(`sigma1_le_sigma1₃`) and the diamond parameter `diag(1, 4)`. -/
def Sigma1₃ : Submonoid (Matrix (Fin 2) (Fin 2) K₃) where
  carrier := {g | g ∈ Sigma0' K₃ γ₉ γ₉_lt_one ∧ Valued.v (g 1 1 - 1) ≤ γ₃}
  one_mem' := by
    refine ⟨one_mem _, ?_⟩
...
/-- Membership in `Σ₁(3)`: the `Σ₀'` conditions plus `v(g₁₁ − 1) ≤ v(3)`. -/
theorem mem_sigma1₃_iff {g : Matrix (Fin 2) (Fin 2) K₃} :
    g ∈ Sigma1₃ ↔ g ∈ Sigma0' K₃ γ₉ γ₉_lt_one ∧
      Valued.v (g 1 1 - 1) ≤ γ₃ := Iff.rfl

/-- The mod-`9` monoid sits inside the wide one. -/
theorem sigma1_le_sigma1₃ : Sigma1 ≤ Sigma1₃ :=
  fun _ hg => ⟨hg.1, le_trans hg.2 γ₉_le_γ₃⟩
...
`JacobsSlash.tsum_binomialCoeff_eq_unitPow`, i.e. what licenses the exponent-density proof of
