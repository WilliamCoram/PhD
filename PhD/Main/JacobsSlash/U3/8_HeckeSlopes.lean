...
-/
import PhD.Main.JacobsSlash.U3.«7_Fredholm»
...
The extension-field endgame of the fork.  `K₃ = ℚ₃` contains no primitive cube root of
unity (`x² + x + 1 ≡ (x+2)² mod 3`, discriminant of odd valuation), so the eigenblock
factorisation of [Jacobs, (2.1.14) + Lemma 1.15] — whose blocks `M₁,₁, M₂,₂, M₃,₃`
carry `ω`-scalars — lives over an extension.  This file states it in maximal
generality and then witnesses it concretely:
...
* **Abstract layer** — for any complete ultrametric `NontriviallyNormedField L`, any
  ...
  factor is the (proved) `JacobsSlash.unitSlope_newtonPolygon₀OfPowerSeries_M22op` at
  `L`: its Newton polygon has unit slopes `1/2, 3/2, 5/2, …` — [Jacobs, Cor 2.16] about
  `U₃`.  (A "Newton polygon of a product" merge theorem is deliberately NOT stated: the
  thesis proves no such result, stopping at the per-factor statement.)
...
/-- **The transcribed matrix commutes with isometric embeddings, entrywise**: each block is
one of the `ε`-operators, whose matrix is a coefficient of the corresponding `h_{i,j}`
(`map_h01 … map_h21`). -/
theorem matrixCoeff_U3MatrixOp_map (f : K₃ →+* L) (hf : ∀ x, ‖f x‖ = ‖x‖) {t : K₃}
    (ht : ‖t‖ < 1) (p q : Fin 3 × ℕ) :
    matrixCoeff (U3MatrixOp (norm_three_lt_one_of_isometry f hf)
        (norm_map_weight_lt_one f hf ht) (map_ν₃_near f hf)) p q
      = f (matrixCoeff (U3MatrixOp norm_three_lt_one ht ν₃_near) p q) := by
  obtain ⟨a, j⟩ := p
  obtain ⟨b, i⟩ := q
  simp only [U3MatrixOp, matrixCoeff_blockOp]
  ...
  · rw [matrixCoeff_epsOp01, matrixCoeff_epsOp01, ← map_h01 f hf,
      MvPowerSeries.coeff_map]
  · rw [matrixCoeff_epsOp02, matrixCoeff_epsOp02, ← map_h02 f hf,
      MvPowerSeries.coeff_map]
  · rw [matrixCoeff_epsOp10, matrixCoeff_epsOp10, ← map_h10 f hf,
      MvPowerSeries.coeff_map]
  · exact (map_zero f).symm
  · rw [matrixCoeff_epsOp12, matrixCoeff_epsOp12, ← map_h12 f hf,
      MvPowerSeries.coeff_map]
  · rw [matrixCoeff_epsOp20, matrixCoeff_epsOp20, ← map_h20 f hf,
      MvPowerSeries.coeff_map]
  · rw [matrixCoeff_epsOp21, matrixCoeff_epsOp21, ← map_h21 f hf,
      MvPowerSeries.coeff_map]
  · exact (map_zero f).symm
...
`K₃`-headline, twist-free in this fork), `TateFredholm.charPowerSeries_map`
(matrix-level base change, `isCompactoid_U3MatrixOp` supplying summability) and
`matrixCoeff_U3MatrixOp_map` (the six generating functions map entrywise,
`PhD.Main.JacobsSlash.«3_BaseChange»`). -/
theorem map_charPowerSeriesU3 (f : K₃ →+* L) (hf : ∀ x, ‖f x‖ = ‖x‖) (t : K₃)
    (ht : ‖t‖ < 1) :
    PowerSeries.map f (charPowerSeriesU3 t ht)
      = charPowerSeries (U3MatrixOp (norm_three_lt_one_of_isometry f hf)
          (norm_map_weight_lt_one f hf ht) (map_ν₃_near f hf)) := by
  rw [charPowerSeriesU3_eq_U3MatrixOp]
  exact (TateFredholm.charPowerSeries_map f hf
    (isCompactoid_U3MatrixOp norm_three_lt_one ht ν₃_near)
    (matrixCoeff_U3MatrixOp_map f hf ht)).symm
...
`JacobsSlash.unitSlope_newtonPolygon₀OfPowerSeries_M22op` at `L` — so this theorem is
[Jacobs, Cor 2.16] as a statement about `U₃`.  Assembly: `map_charPowerSeriesU3` +
`JacobsSlash.charPowerSeries_U3MatrixOp` at `L`. -/
theorem charPowerSeriesU3_factorisation (f : K₃ →+* L) (hf : ∀ x, ‖f x‖ = ‖x‖)
    (ω : L) (hω : ω ^ 2 + ω + 1 = 0) (t : K₃) (ht : ‖t‖ < 1) :
    PowerSeries.map f (charPowerSeriesU3 t ht)
      = charPowerSeries (M11op (norm_three_lt_one_of_isometry f hf)
            (norm_map_weight_lt_one f hf ht) (map_ν₃_near f hf)) *
          (charPowerSeries (M22op ω hω (norm_three_lt_one_of_isometry f hf)
              (norm_map_weight_lt_one f hf ht) (map_ν₃_near f hf)) *
            charPowerSeries (M33op ω hω (norm_three_lt_one_of_isometry f hf)
              (norm_map_weight_lt_one f hf ht) (map_ν₃_near f hf))) :=
  (map_charPowerSeriesU3 f hf t ht).trans <|
    charPowerSeries_U3MatrixOp (ω := ω) (hω := hω)
...
theorem charPowerSeriesU3_factorisation_L₃ (t : K₃) (ht : ‖t‖ < 1) :
    PowerSeries.map ι₃ (charPowerSeriesU3 t ht)
      = charPowerSeries (M11op (norm_three_lt_one_of_isometry ι₃ norm_ι₃)
            (norm_map_weight_lt_one ι₃ norm_ι₃ ht) (map_ν₃_near ι₃ norm_ι₃)) *
          (charPowerSeries (M22op ω₃ ω₃_sq_add_ω₃_add_one
              (norm_three_lt_one_of_isometry ι₃ norm_ι₃)
              (norm_map_weight_lt_one ι₃ norm_ι₃ ht) (map_ν₃_near ι₃ norm_ι₃)) *
            charPowerSeries (M33op ω₃ ω₃_sq_add_ω₃_add_one
              (norm_three_lt_one_of_isometry ι₃ norm_ι₃)
              (norm_map_weight_lt_one ι₃ norm_ι₃ ht) (map_ν₃_near ι₃ norm_ι₃))) :=
