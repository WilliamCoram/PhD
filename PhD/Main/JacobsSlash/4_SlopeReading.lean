...
`PhD.Main.NewtonPolygons.OfSlopes`** (moved there on 2026-08-20, slopes-hecke board A0); this file
...
`ϖ₃` is a **definition**, not a hypothesis: `PseudoUniformizer` (`PhD/Main/TateFredholm/00_Tate.lean`)
...
`isNewtonPolygonOf_val_charCoeff_M22op` would be vacuous.  See the `PhD.Main.JacobsSlash.U3Data` module
...
Newton-polygon reading); the blueprint Definition 1 via `PhD.Main.NewtonPolygons.Spec`.
-/
...
See `PhD.Main.JacobsSlash.U3Data`'s module docstring for the full account. -/
section CaseB
...
`PhD.Main.JacobsSlash.DiamondW`) and `U₃`'s matrix is identified in `PhD.Main.JacobsSlash.U3.Matrix`
...
formalised — it needs [Jacobs, Lemma 2.9]; see `PhD.Main.JacobsSlash.DiamondW`'s module header. -/
