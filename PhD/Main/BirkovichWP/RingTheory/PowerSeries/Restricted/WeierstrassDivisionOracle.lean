...
uniqueness live in the shared core `PhD.Main.ForMathlib.…PowerSeries.Restricted.WeierstrassDivision`).

This engine is **subsumed** by the extension-free Martin proof
`PhD.Main.ForMathlib.…PowerSeries.Restricted.MulWeierstrassDivision.weierstrassDivision_exists_of_isMulDistinguished`
...
`PhD.Main.BirkovichWP`, and is not part of the mathlib-bound canonical development.
-/
