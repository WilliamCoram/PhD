...
import PhD.Main.TateFredholm.«00_Compose»
...
action theory (`PhD/Main/QMF/Weight/03_SlashAction.lean`) is proved once.  The public notion of
a weight is `QMF.AnalyticWeight U S ρ` (`PhD/Main/QMF/Weight/04_Char.lean`: an honest character
...
`PhD/Main/JacobsSlash/`), the classical algebraic weights (`κ(u) = u^(n+2)`,
`PhD/Main/QMF/Weight/06_Algebraic.lean`) and honest character expansions
(`PhD/Main/QMF/Weight/04_Char.lean`) are all instances.
...
* `QMF.WeightSeries.genFun` — `κcol·(linSeries γ)⁻¹·(quadSeries γ)⁻¹`, the generating
  ...
  (`PhD/Main/QMF/Weight/03_SlashAction.lean`).
-/
