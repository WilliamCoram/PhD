...
import PhD.Main.QMF.Slash.«03_WeightModule»
import PhD.Main.QMF.Slash.«04_HeckeMatrix»

/-!
# Quaternionic modular forms with the classical right slash

Right-handed mirror of `PhD/Main/QMF/03_Quaternionic.lean` (+ the `etaAdelic` layer of
`PhD/Main/QMF/04_UpiElement.lean`): the specialisation of the slash theory to a quaternion
...
* `spaceSlash_eq_space` — the FLT-facing agreement corollary: under the pointwise
  ...
  left-action space of `PhD/Main/QMF/03_Quaternionic.lean`.
-/
...
space *equals* the left-action space of `PhD/Main/QMF/03_Quaternionic.lean`.  Instantiation of
