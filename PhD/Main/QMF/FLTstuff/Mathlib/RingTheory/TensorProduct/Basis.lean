...
-- PORT (v4.33): needed for the `Fintype (Module.Free.ChooseBasisIndex R A)` instance,
...
@[simp]
lemma Module.Basis.rightBaseChange_apply [DecidableEq ι] (b : Module.Basis ι R A) (i) :
...
variable {R : Type*} {M : Type*} {B : Type*} {ι : Type*} [CommSemiring R]
variable [AddCommMonoid M] [Module R M] [Fintype ι] [CommSemiring B] [Algebra R B]

/-- **Module version of `Module.Basis.rightBaseChange`**: an `R`-basis of a module `M` lifts to
a `B`-basis of the base change `M ⊗[R] B`.  (`Module.Basis.rightBaseChange`, in
`PhD/Main/QMF/FLTstuff/Mathlib/RingTheory/TensorProduct/Basis.lean`, asks for `M` to be a
*commutative* `R`-algebra; the construction only uses the module structure, which is what a
noncommutative `D` such as a quaternion algebra needs.) -/
noncomputable
def Module.Basis.rightBaseChange' [DecidableEq ι] (b : Module.Basis ι R M) :
    Module.Basis ι B (M ⊗[R] B) where
  repr :=
    let comm : B ⊗[R] M ≃ₗ[B] M ⊗[R] B :=
      TensorProduct.RightActions.Module.TensorProduct.comm R B M
    let π : B ⊗[R] M ≃ₗ[B] (ι → B) :=
      (TensorProduct.AlgebraTensorModule.congr (LinearEquiv.refl B B) b.equivFun).trans
        (TensorProduct.piScalarRight _ _ _ _)
    let finite : (ι →₀ B) ≃ₗ[B] (ι → B) := Finsupp.linearEquivFunOnFinite B B ι
    comm.symm.trans π |>.trans finite.symm

@[simp]
lemma Module.Basis.rightBaseChange'_repr [DecidableEq ι] (b : Module.Basis ι R M) (i) (x : B) :
    (b.rightBaseChange' (B := B)).repr (b i ⊗ₜ x) = Finsupp.single i x := by
  have : ∑ j : ι, (Pi.single i (1 : R) : ι → R) j • (b j) = b i := by
    conv =>
      lhs
      arg 2
...
@[simp]
lemma Module.Basis.rightBaseChange'_apply [DecidableEq ι] (b : Module.Basis ι R M) (i) :
    (b.rightBaseChange' (B := B)) i = b i ⊗ₜ (1 : B) := by
  rw [apply_eq_iff]
  exact rightBaseChange'_repr b i 1
