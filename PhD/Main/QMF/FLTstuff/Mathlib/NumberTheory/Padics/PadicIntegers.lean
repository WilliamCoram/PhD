...
Authors: Yaël Dillies, Kevin Buzzard, Pietro Monticone, Ruben Van de Velde
-/
...
PARTIAL PORT (T016f-1) of `FLT.Mathlib.NumberTheory.Padics.PadicIntegers`: only
`PadicInt.coe_coeRingHom`, the one declaration needed by
`PhD.Main.QMF.FLTstuff.HaarMeasure.MeasurableSpacePadics`, is ported. Everything else in the FLT
file is either already in mathlib (`coe_injective`, `coe_inj`, `Infinite ℤ_[p]`, `nnnorm_p`,
`isOpenEmbedding_coe`, …) or is not needed by this tranche.

-- PORT (T016f-2 extension): `PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.Padic` needs the rest of the
FLT file, so the remainder is now ported below in the original order. The only omission is FLT's
`attribute [simp] coe_eq_zero`, which mathlib now carries on the declaration itself.
-/
...
lemma closure_nonZeroDivisors_padicInt :
  ...
  ext
  -- PORT (v4.33): `simpa using hyzx.symm` no longer closes this. `Units.val_mk0` is no longer
  -- `@[simp]`, and adding it does not help either: `ℤ_[p]` unfolds to `{x : ℚ_[p] // ‖x‖ ≤ 1}`,
  -- so `↑↑(⟨y, hy⟩ : ℤ_[p]⁰)` is not type-correct at `implicit` transparency and neither `simp`
  -- nor `rw` can rewrite under it. Splitting the division and finishing with a term (`exact`
  -- works at default transparency) avoids the problem.
