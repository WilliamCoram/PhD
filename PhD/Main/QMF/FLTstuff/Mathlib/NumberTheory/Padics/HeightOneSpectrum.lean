...
theorem intValuation_eq_padicValuation_iff_multiplicity_eq_multiplicity {x : R}
  ...
  simp [intValuation_eq_exp_neg_multiplicity _ hx, padicValuation, hx,
...
theorem valuation_apply_coe_eq_padicValuation (x : R)
  ...
  · rw [intValuation_eq_padicValuation_iff_multiplicity_eq_multiplicity v hx]
    exact multiplicity_eq_of_emultiplicity_eq <| emultiplicity_eq_emultiplicity_iff.2
      -- PORT (v4.33): `simp [primesEquiv]` now unfolds the Equiv to a structure literal,
      -- which blocks `pow_natGenerator_dvd_iff`; reduce the coercion by `rfl` instead.
      fun n ↦ by
        simp [show ((primesEquiv v : Nat.Primes) : ℕ) = natGenerator v from rfl,
          pow_natGenerator_dvd_iff v n, ← Ideal.map_pow]
