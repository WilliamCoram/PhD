...
PORT (T016f-1) of `FLT.Mathlib.MeasureTheory.Haar.Extension`. The `IsSES` structure itself and
`inducedMeasure` (with `isHaarMeasure_inducedMeasure` and `inducedMeasure_lt_of_injOn`) have
landed in mathlib as `Mathlib.Topology.Algebra.Group.Extension` /
`Mathlib.MeasureTheory.Measure.Haar.Extension`; only the two `not_injOn_*` results below are
still FLT-only.
-/
