...
PARTIAL PORT (T016d part 2a) of `FLT.Mathlib.Algebra.Algebra.Hom`: the declarations
`SemialgHom.prod`, `SemialgHom.prodMap` and `SemialgHom.compAlgHom` are not needed by
`PhD.Main.QMF.FLTstuff.DedekindDomain.Completion.BaseChange` and are omitted.

-- PORT (T016d-2b extension): `SemialgHom.compAlgHom`, `SemialgHom.prod` and
`SemialgHom.prodMap` appended (needed by
`PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.Algebra.Hom`); the file is now a full port.
-/
...
-- PORT (T016d-2b extension): the following three declarations are appended from the FLT source.
