...
`Pi.semialgHom_apply` are needed by `PhD.Main.QMF.FLTstuff.DedekindDomain.Completion.BaseChange`;
...
`PhD.Main.QMF.FLTstuff.NumberField.Completion.Infinite` and
`PhD.Main.QMF.FLTstuff.NumberField.InfiniteAdeleRing`).
-/
