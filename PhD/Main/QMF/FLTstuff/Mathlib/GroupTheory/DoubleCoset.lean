...
def DoubleCoset.toMapIdPreimage {G : Type*} [Group G] (H K₁ K₂ : Subgroup G) (eK : K₁ ≤ K₂)
  ...
  · suffices mk H K₂ (g * ↑k) = mk H K₂ g by exact this
...
lemma DoubleCoset.toMapIdPreimage_surjective
  ...
  suffices h : ∀ x, mk H K₂ x = mk H K₂ g → ∃ a ∈ K₂, mk H K₁ (g * a) = mk H K₁ x by
    rintro ⟨b, hb⟩
    obtain ⟨x, rfl⟩ := DoubleCoset.mk_surjective H K₁ b
    obtain ⟨a, ha, e⟩ := h x hb
...
def _root_.QuotientGroup.quotientConjActEquivImageMulSingleton
  ...
  · intro a b e
    simpa [Subtype.ext_iff, QuotientGroup.eq, QuotientGroup.leftRel_apply,
  ...
  · refine ⟨Quotient.ind₂ ?_, Quotient.lift_surjective _ _ ?_⟩
    · simp [Function.Surjective, Subtype.ext_iff, -Set.image_mul_right]
    · simp [Subtype.ext_iff, QuotientGroup.eq, Subgroup.mem_pointwise_smul_iff_inv_smul_mem,
        -smul_mul', ConjAct.toConjAct_inv_smul', mul_assoc]
...
lemma DoubleCoset.sum_filter_map_eq_relIndex_eq_relIndex
    ...
    (g : H＼G／K₂) [Fintype (H＼G／K₁)] [DecidableEq (H＼G／K₂)] [K₁.IsFiniteRelIndex K₂] :
      ...
      have H₃ : (ConjAct.toConjAct b)⁻¹ • H = H := by
        rw [← ConjAct.toConjAct_inv]
        exact Subgroup.conjAct_pointwise_smul_eq_self (Subgroup.le_normalizer (inv_mem hb))
      ...
      K₁.relIndex K₂ := by
  classical
  obtain ⟨g, rfl⟩ := DoubleCoset.mk_surjective H K₂ g
  trans ∑ a, Set.ncard (toMapIdPreimage H K₁ K₂ eK g ⁻¹' {a})
  · refine ((Finset.sum_attach _ _).symm.trans ?_)
    refine (Equiv.sum_comp (M := ℕ) (Equiv.subtypeEquivRight
      (q := fun a ↦ DoubleCoset.map H K₁ H K₂ (.id _) le_rfl eK a = mk H K₂ g)
      Finset.mem_filter_univ).symm _).symm.trans ?_
...
lemma DoubleCoset.σ_exists :
  ...
  exact ⟨fun a ↦ τ (σ a), fun a ↦ x ↑a, y, by grind, by grind, by grind, by grind, by simp⟩
