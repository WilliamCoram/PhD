...
PORT (T016g) of `FLT.Mathlib.Topology.HomToDiscrete`.
-/
...
lemma inter_Discrete {A : Type*} [TopologicalSpace A] (X Y : Set A)
  ...
  -- PORT (v4.33): the closing `aesop` no longer discharges this; done by hand.
  obtain ⟨hmem, huniq⟩ := Ueq
  ext ⟨b, hbY, hbX⟩
  constructor
  · intro hb
    have h' : (⟨b, hbX⟩ : ↑X) = ⟨a, InX⟩ := huniq ⟨b, hbX⟩ hb
    have hba : b = a := by simpa using h'
    exact Subtype.ext hba
  · intro hb
    have hb' : (⟨b, ⟨hbY, hbX⟩⟩ : ↑(Y ∩ X)) = ⟨a, ⟨InY, InX⟩⟩ := hb
    have hba : b = a := Subtype.ext_iff.mp hb'
    subst hba
    exact hmem
