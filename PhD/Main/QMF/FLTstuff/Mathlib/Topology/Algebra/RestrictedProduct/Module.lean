...
def _root_.LinearEquiv.restrictedProductPi [Fintype n] :
  ...
  toFun x j := map (fun i y ↦ y j)
    (by
      ...
      simp only [SetLike.mem_coe, piSubringSubmodule] at hr
      exact hr j (Set.mem_univ j))
    x
  invFun x := ⟨fun i j ↦ x j i, by
    filter_upwards [Filter.eventually_all.mpr fun j ↦ (x j).eventually] with i hi
    exact fun j _ ↦ hi j⟩
  map_add' x y := rfl
  map_smul' x y := rfl
...
lemma isOpen_piSubringSubmodule [Finite n] (hOpen : ∀ i, IsOpen (B i : Set (R i))) (i : ι) :
  ...
  exact isOpen_set_pi Set.finite_univ fun j _ => hOpen i
...
def _root_.ContinuousLinearEquiv.restrictedProductPi [Fintype n]
  ...
  continuous_toFun := by
    apply continuous_pi
    intro j
    exact Continuous.restrictedProduct_congrRight
      (Filter.Eventually.of_forall fun i ↦
        (fun r hr ↦ hr j (Set.mem_univ j) : Set.MapsTo (fun y ↦ y j)
          (piSubringSubmodule B n i : Set (n → R i)) (B i : Set (R i))))
      fun _ ↦ continuous_apply j
