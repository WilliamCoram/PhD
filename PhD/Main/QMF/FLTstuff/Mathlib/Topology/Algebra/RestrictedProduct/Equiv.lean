...
`PhD.Main.QMF.FLTstuff.DedekindDomain.FiniteAdeleRing.BaseChange` needs. The matrix, units
...
file (needed by `PhD.Main.QMF.FLTstuff.NumberField.AdeleRing`). The `MulEquiv`/`LinearEquiv`
...
`PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.AddEquiv`).
-/
...
-- `PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.RestrictedProduct.TopologicalSpace`.
