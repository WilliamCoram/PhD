...
PARTIAL PORT (T016d part 2a) of `FLT.Hacks.RightActionInstances`: the right-action
(now-restored) `LinearMap.baseChange` / `LinearEquiv.baseChange` / `AlgebraMap.baseChange` /
`Algebra.TensorProduct.basis` definitions are
not needed by `PhD.Main.QMF.FLTstuff.DedekindDomain.Completion.BaseChange` and are omitted.
The FLT-only lemmas `IsModuleTopology.Module.topologicalRing` and
`IsModuleTopology.topologicalAddGroup` used by the original have landed in mathlib as
`IsModuleTopology.isTopologicalRing` and `IsModuleTopology.topologicalAddGroup`.
-/
...
-- PORT (T016f-2b extension): the baseChange block (LinearMap.baseChange,
-- LinearMap.baseChange_comp, LinearEquiv.baseChange, AlgebraMap.baseChange,
-- Algebra.TensorProduct.basis), omitted by the original partial port, is needed by
-- `HaarMeasure/HaarChar/AdeleRing.lean`.
...
scoped instance [TopologicalSpace A] [IsTopologicalRing A] [Module.Finite R B] :
  ...
  -- PORT (v4.33): FLT's `IsModuleTopology.Module.topologicalRing` landed in mathlib as
  -- `IsModuleTopology.isTopologicalRing`.
  IsModuleTopology.isTopologicalRing A _

-- PORT (T016f-2b extension): the `LocallyCompactSpace` scoped instance, omitted by the
-- original partial port, is needed for `B ⊗[K] 𝔸_K^f` in
-- `HaarMeasure/HaarChar/FiniteAdeleRing.lean`.
