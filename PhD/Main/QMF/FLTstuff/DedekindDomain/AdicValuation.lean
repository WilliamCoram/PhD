...
import PhD.Main.QMF.FLTstuff.Mathlib.RingTheory.LocalRing.MaximalIdeal.Basic
import PhD.Main.QMF.FLTstuff.Mathlib.RingTheory.Valuation.ValuationSubring
...
lemma exists_ofAdd_natCast_of_le_one {x : ℤᵐ⁰} (hx : x ≠ 0) (hx' : x ≤ 1) :
  ...
  norm_cast at hx'
  -- PORT (v4.33): the old proof abused `Multiplicative ℤ ≡ ℤ` defeq, which `rw` now rejects;
  -- go through `toAdd` explicitly instead.
  replace hx' : Multiplicative.toAdd x ≤ 0 := by
    rw [← toAdd_one, Multiplicative.toAdd_le]; exact hx'
  obtain ⟨k, hk⟩ := Int.eq_ofNat_of_zero_le (Int.neg_nonneg_of_nonpos hx')
  refine ⟨k, ?_⟩
  rw [← hk, neg_neg, ofAdd_toAdd]
...
lemma exists_ofAdd_natCast_lt {x : ℤᵐ⁰} (hx : x ≠ 0) :
  ...
  -- PORT (v4.33): the old proof abused `Multiplicative ℤ ≡ ℤ` defeq (`y.natAbs`, `inv_mabs_le`);
  -- go through `toAdd` explicitly instead.
  refine ⟨(Multiplicative.toAdd y).natAbs, lt_of_le_of_lt ?_ hyx⟩
  rw [WithZero.coe_le_coe]
  calc Multiplicative.ofAdd (-((Multiplicative.toAdd y).natAbs : ℤ))
      ≤ Multiplicative.ofAdd (Multiplicative.toAdd y) := by
        rw [Multiplicative.ofAdd_le, Int.natCast_natAbs]
        exact neg_abs_le _
    _ = y := ofAdd_toAdd y
...
lemma exists_adicValued_mul_sub_le {a b : A} {γ : WithZero (Multiplicative ℤ)} (hγ : γ ≠ 0)
  ...
  rw [intValuation_eq_exp_neg_multiplicity _ hnz, WithZero.exp_le_exp, neg_le_neg_iff,
    Int.ofNat_le] at hle
  have hm : emultiplicity v.asIdeal (Ideal.span {a}) ≤ n :=
    le_of_eq_of_le
      (emultiplicity_eq_of_valuation_eq_ofAdd v <| intValuation_eq_exp_neg_multiplicity v hnz)
      (ENat.natCast_le_natCast.mpr hle)
  have hb : b ∈ v.asIdeal ^ multiplicity v.asIdeal (Ideal.span {a}) := by
    rwa [← intValuation_le_pow_iff_mem, ← intValuation_eq_exp_neg_multiplicity _ hnz]
  -- Now make use of
...
theorem inertiaDeg_asIdeal_completionIdeal :
  ...
  have f : (A ⧸ v.asIdeal) ≃ₗ[A ⧸ v.asIdeal]
    ...
    -- PORT (v4.33): `rw [Algebra.smul_def]` no longer matches the quotient-module `•`
    -- (defeq-only instance path); assemble the equality at the term level instead.
    map_smul' := fun x y =>
      (map_mul (ResidueFieldEquivCompletionResidueField K v) x y).trans
        (Algebra.smul_def (A := (adicCompletionIntegers K v) ⧸ completionIdeal K v) x
          ((ResidueFieldEquivCompletionResidueField K v) y)).symm
  }
...
theorem eq_pow_uniformizer_mul_unit {x : v.adicCompletionIntegers K} (hx : x ≠ 0)
  ...
  -- PORT (v4.33): `IsUnit.unit_spec` no longer fires inside `simp` here (defeq-only instance
  -- path); destructure the `IsUnit` witness instead and combine the powers via `zpow_add₀`.
  have ha : IsUnit a := ValuationSubring.isUnit_of_valued_eq_one a hpow
  obtain ⟨u, hu⟩ := ha
  refine ⟨m.toNat, u, Subtype.ext ?_⟩
  have hπ0 : ((π : v.adicCompletionIntegers K) : v.adicCompletion K) ≠ 0 := by
    simp [uniformizer_ne_zero hπ]
  have hu' : ((↑u : v.adicCompletionIntegers K) : v.adicCompletion K) =
      ↑π ^ (-m) * ↑x := by rw [hu]
  rw [MulMemClass.coe_mul, SubmonoidClass.coe_pow, hu',
    ← zpow_natCast ((π : v.adicCompletionIntegers K) : v.adicCompletion K),
    m.toNat_of_nonneg hm₀, ← mul_assoc, ← zpow_add₀ hπ0, add_neg_cancel, zpow_zero, one_mul]
