...
theorem Module.FinitePresentation.exists_fin_exact (R : Type*) (M : Type*)
  ...
  have h₁ : LinearMap.range f = K := by
    -- PORT (v4.33): `Finset.setOf_mem` deprecated in favour of `Finset.setOfPred_mem`.
...
lemma tensorPi_equiv_piTensor_apply (m : M) (n : ∀ i, N i) :
  ...
  · rw [← LinearEquiv.eq_symm_apply]
    ...
    ext i
    -- PORT (v4.33): `LinearEquiv.rTensor_symm_tmul` deprecated; use `symm_rTensor`+`rTensor_tmul`.
    simp only [LinearEquiv.piCongrRight_apply, ← LinearEquiv.symm_rTensor,
      LinearEquiv.rTensor_tmul, LinearEquiv.symm_symm,
