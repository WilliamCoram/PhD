...
import PhD.Main.QMF.FLTstuff.Mathlib.Algebra.IsDirectLimit
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.RestrictedProduct.Basic
import PhD.Main.QMF.FLTstuff.DedekindDomain.FiniteAdeleRing.IsDirectLimitRestricted
...
lemma lTensorLeft_tmul (R M : Type*) [CommRing R] [CommRing M] [Algebra R M] {ι : Type*}
  ...
  -- PORT (v4.33): `rw [mapAlongLinearMap_apply]` now fails (kabstract rejects the defeq-only
  -- occurrence); the subsequent defeq `change` makes it unnecessary.
  rw [lTensorLeft, LinearMap.liftBaseChange_tmul, smul_apply]
  change m • (_ ⊗ₜ[R] _) = _
...
noncomputable def lTensorPrincipalEquiv :
  ...
  let g3 : (Π i, M ⊗[R] N' i) ≃ₗ[R] Πʳ i, [M ⊗[R] N i, rangeLTensor R M N L i]_[𝓟 S] := {
    ...
    right_inv x := by
      ext i
      by_cases h : i ∈ S <;> simp [h]
    -- PORT (v4.33): `simp [h]` no longer sees through the componentwise `+`/`•` on the target
    -- restricted product (and the scoped `Classical` instance is invisible to `simp`/`show`
    -- inside these field tactic blocks); finish each branch with fully-explicit
    -- `dif_pos`/`dif_neg` facts (instance and motives pinned) and a defeq `congrArg` step.
    map_add' x y := by
      ext i
      by_cases h : i ∈ S
      · simp [h]
        have hx := @dif_pos (i ∈ S) (Classical.propDecidable _) h (M ⊗[R] N i)
          (fun h => (↑((gEquiv i h) (x i)) : M ⊗[R] N i)) (fun h => (gEquiv' i h) (x i))
        have hy := @dif_pos (i ∈ S) (Classical.propDecidable _) h (M ⊗[R] N i)
          (fun h => (↑((gEquiv i h) (y i)) : M ⊗[R] N i)) (fun h => (gEquiv' i h) (y i))
        exact (congrArg₂ (· + ·) hx hy).symm
      · simp [h]
        have hx := @dif_neg (i ∈ S) (Classical.propDecidable _) h (M ⊗[R] N i)
          (fun h => (↑((gEquiv i h) (x i)) : M ⊗[R] N i)) (fun h => (gEquiv' i h) (x i))
        have hy := @dif_neg (i ∈ S) (Classical.propDecidable _) h (M ⊗[R] N i)
          (fun h => (↑((gEquiv i h) (y i)) : M ⊗[R] N i)) (fun h => (gEquiv' i h) (y i))
        exact (congrArg₂ (· + ·) hx hy).symm
    map_smul' a x := by
      ext i
