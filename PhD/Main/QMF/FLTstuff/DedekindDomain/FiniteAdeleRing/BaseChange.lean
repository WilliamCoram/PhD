...
-/
import PhD.Main.QMF.FLTstuff.DedekindDomain.Completion.BaseChange
import PhD.Main.QMF.FLTstuff.DedekindDomain.FiniteAdeleRing.TensorRestrictedProduct
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.RestrictedProduct.Module
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.Algebra.Hom
import PhD.Main.QMF.FLTstuff.Mathlib.LinearAlgebra.Pi
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.RestrictedProduct.TopologicalSpace
import Mathlib.RingTheory.DedekindDomain.FiniteAdeleRing
import Mathlib.RingTheory.Flat.TorsionFree
import PhD.Main.QMF.FLTstuff.Mathlib.RingTheory.DedekindDomain.FiniteAdeleRing
...
lemma tendsTo_comap_cofinite [FaithfulSMul A B] :
  ...
  -- PORT (v4.33): `FractionRing.instFaithfulSMul A B` is now an anonymous mathlib instance.
  have : FaithfulSMul A (FractionRing B) := inferInstance
...
-- PORT (v4.33): the pointwise `SMul (K_v) (L_w)` family is registered on `v.Extension B`;
-- unifying it with the `{ a // under A a = b }` form needs unfolding the `Extension` def,
-- which the v4.33 defeq refuses at instance transparency.
...
-- PORT (v4.33): relax defeq so the `Extension`-typed simp/rw patterns match their
-- subtype-shaped occurrences (same reason as for `ComapFiberwiseSMul` above); the relaxed
-- search needs a larger instance-synthesis budget.
set_option backward.isDefEq.respectTransparency false in
...
-- PORT (v4.33): the FLT source names the (module-system-mangled) instance
-- `RestrictedProduct.instModuleCoe_fLT` directly; synthesise it via `inferInstanceAs` at the
-- restricted-product-scalars type instead. The nested synthesis needs the FLT lakefile's
-- `maxSynthPendingDepth = 3` (mathlib parity) and the relaxed defeq, per-declaration.
...
noncomputable local instance : Module 𝔸ᶠ[A, K]
  ...
  inferInstanceAs (Module
    (Πʳ v : HeightOneSpectrum A, [v.adicCompletion K, v.adicCompletionIntegers K]) _)
