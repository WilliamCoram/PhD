...
-/
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.Module.TensorProduct
...
lemma TensorProduct.localcomponent_comp_apply (p : HeightOneSpectrum R)
  ...
  -- PORT (v4.33): `simp` now stops with the `restrictScalars K` wrappers in place, hiding the
  -- `𝔸_K^f`-linearity of `φ` from the simp set; peel them off with a defeq `congr`+`map_smul`.
  congr 1
  exact (map_smul φ (localIdempotent R K p) _).symm
