...
-/
import PhD.Main.QMF.FLTstuff.Mathlib.RingTheory.Localization.BaseChange -- removing this breaks a simp proof
...
lemma valuation_comap (w : HeightOneSpectrum B) (x : K) :
  ...
  -- PORT (v4.33): `valuation_def` now unfolds to `extendToLocalization`, on which the
  -- `← intValuation_comap` rewrite no longer fires; go through `valuation_of_algebraMap` instead.
  simp [map_div₀, ← IsScalarTower.algebraMap_apply A K L, IsScalarTower.algebraMap_apply A B L,
...
lemma LinearEquivTensorProduct_symm_one_tmul (b : B) :
    ...
    (algebraMap _ _ b) := by
  -- PORT (v4.33): `IsLocalizedModule.iso_mk_one` no longer fires inside `simp` here
  -- (defeq-only instance path on the `1`); apply it as a term instead.
  have := IsIntegralClosure.isLocalization A K L B
  have : IsLocalizedModule (nonZeroDivisors A) (IsScalarTower.toAlgHom A B L).toLinearMap :=
    inferInstance
  simp [LinearEquivTensorProduct]
  exact IsLocalizedModule.iso_mk_one (nonZeroDivisors A)
    ((IsScalarTower.toAlgHom A B L).toLinearMap) b
...
/-- The canonical `B`-linear isomorphism `L ⊗ M ≅ B ⊗ M` for any `K`-module `M`. -/
