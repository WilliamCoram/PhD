...
-/
import PhD.Main.QMF.FLTstuff.DedekindDomain.AdicValuation
import PhD.Main.QMF.FLTstuff.DedekindDomain.IntegralClosure
import PhD.Main.QMF.FLTstuff.Mathlib.Algebra.Algebra.Bilinear
import PhD.Main.QMF.FLTstuff.Mathlib.Algebra.Algebra.Pi
import PhD.Main.QMF.FLTstuff.Mathlib.Algebra.Module.Submodule.Basic
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.Module.ModuleTopology
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.UniformRing
import Mathlib.RingTheory.Flat.FaithfullyFlat.Basic
import Mathlib.RingTheory.Valuation.Discrete.RankOne
import Mathlib.Topology.Algebra.Valued.NormedValued
import PhD.Main.QMF.FLTstuff.Mathlib.RingTheory.TensorProduct.Basis
import Mathlib.NumberTheory.RamificationInertia.Basic
-- PORT (v4.33): FLT's ε-δ proof of `adicValued.continuous_algebraMap` landed in mathlib as
-- `IsDedekindDomain.HeightOneSpectrum.uniformContinuous_algebraMap_liesOver`.
import Mathlib.NumberTheory.RamificationInertia.Valuation
import Mathlib.RingTheory.PicardGroup
import Mathlib.RingTheory.SimpleRing.Principal
import Mathlib.Topology.Algebra.Module.FiniteDimension
import PhD.Main.QMF.FLTstuff.Mathlib.RingTheory.DedekindDomain.AdicValuation
...
lemma adicValued.continuous_algebraMap
   (w : HeightOneSpectrum B) (hvw : w.under A = v) :
    Continuous (σ v w) := by
  -- PORT (v4.33): the original ε-δ proof of this lemma landed in mathlib as
  -- `uniformContinuous_algebraMap_liesOver`; use it.
  subst hvw
  have : Module.IsTorsionFree A B := isTorsionFree A K L B
  have : w.asIdeal.LiesOver (w.under A).asIdeal := ⟨rfl⟩
  exact (uniformContinuous_algebraMap_liesOver K L (w.under A) w).continuous
...
noncomputable local instance :
    Valuation.RankOne (Valued.v : Valuation (adicCompletion K v) ℤᵐ⁰) :=
  -- PORT (v4.33): the field-by-field construction landed in mathlib as
  -- `Valuation.IsRankOneDiscrete.rankOne` (for any discrete valuation and any real base > 1);
  -- use it with base 2.
  Valuation.IsRankOneDiscrete.rankOne _ (one_lt_two (α := NNReal))
...
instance {w : v.Extension B} : IsScalarTower (adicCompletionIntegers K v) (adicCompletion K v)
  ...
  -- PORT (v4.33): the mathlib auto-named instance `Submonoid.instIsScalarTowerSubtypeMem`
  -- referenced by FLT no longer exists under that name; prove the tower law directly.
  ⟨fun x y z ↦ mul_smul (x : adicCompletion K v) y z⟩
...
noncomputable local instance : MulAction (v.adicCompletionIntegers K) (v.adicCompletion K) :=
  -- PORT (v4.33): the projection sugar `LieAlgebra.ofAssociativeAlgebra.toMulAction` no longer
  -- resolves; take the same underlying `Module`-instance path explicitly.
  ((inferInstance :
    Module (v.adicCompletionIntegers K) (v.adicCompletion K))).toDistribMulAction.toMulAction
...
noncomputable instance (priority := 1001) [Module.Finite A B] [FiniteDimensional K L] :
  ...
  let integerSubmodule : Submodule (adicCompletionIntegers K v) (adicCompletion L w.1) :=
    ...
    have : IsScalarTower (adicCompletionIntegers K v) (adicCompletionIntegers L w.1)
        (adicCompletion L w.1) :=
      -- PORT (v4.33): `.of_algebraMap_smul fun _ _ ↦ rfl` no longer elaborates here (the
      -- unifier resolves the `SMul` arguments along a different instance path); prove the
      -- associativity law directly.
      ⟨fun x y z ↦ mul_assoc (algebraMap _ (adicCompletion L w.1) x.val) y.val z⟩
...
lemma _root_.WithZero.ofAdd_neg_ofNat_pow (n : ℕ) :
  ...
  congr
  -- PORT (v4.33): `congr` now exposes the additive form via `toAdd`
  -- (`Multiplicative ℤ ≡ ℤ` defeq abuse is no longer accepted by `rw`).
  simp
