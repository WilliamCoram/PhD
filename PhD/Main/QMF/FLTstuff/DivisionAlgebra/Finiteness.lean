...
import PhD.Main.QMF.FLTstuff.DedekindDomain.FiniteAdeleRing.TensorPi
import PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.Ring
import PhD.Main.QMF.FLTstuff.Hacks.RightActionInstances
import PhD.Main.QMF.FLTstuff.Mathlib.LinearAlgebra.Dimension.Constructions
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.NumberField.AdeleRing
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.NumberField.FiniteAdeleRing
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.NumberField.InfiniteAdeleRing
import Mathlib.GroupTheory.DoubleCoset
import Mathlib.RingTheory.Flat.TorsionFree
import Mathlib.RingTheory.SimpleRing.Principal
import Mathlib.RingTheory.TensorProduct.Pi
import PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.AddEquiv
import PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.AdeleRing
import PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.FiniteAdeleRing
import PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.RealComplex
import PhD.Main.QMF.FLTstuff.Mathlib.GroupTheory.DoubleCoset
import PhD.Main.QMF.FLTstuff.Mathlib.LinearAlgebra.Determinant
import PhD.Main.QMF.FLTstuff.Mathlib.LinearAlgebra.TensorProduct.Basis
import PhD.Main.QMF.FLTstuff.Mathlib.MeasureTheory.Haar.Extension
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.HomToDiscrete
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.MetricSpace.ProperSpace.InfinitePlace
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Polish
...
PORT (T016g) of `FLT.DivisionAlgebra.Finiteness`.
-/
...
def DinfTensorPiEquivPiTensorAux :
  ...
  map_smul' x y := by
    -- PORT (v4.33): `simp only [Algebra.smul_def]` no longer rewrites the `x • _` on the
    -- right-hand side, and `rw [h₂]` no longer finds its pattern: the goal contains `↑i`
    -- for `i : InfinitePlace K` (a subtype `def`), so it is not type-correct at `implicit`
    -- transparency and `kabstract` refuses. We chain the same three facts as terms instead.
    ...
    have hkey : tensorPiEquivPiTensor K D InfinitePlace.Completion ((algebraMap ℝ (Dinf K D)) x)
        = (algebraMap ℝ ((i : InfinitePlace K) → D ⊗[K] i.Completion)) x :=
      calc tensorPiEquivPiTensor K D InfinitePlace.Completion ((algebraMap ℝ (Dinf K D)) x)
          = tensorPiEquivPiTensor K D InfinitePlace.Completion
              (1 ⊗ₜ[K] (algebraMap ℝ (InfiniteAdeleRing K) x)) := congrArg _ h₁
        _ = fun (i : InfinitePlace K) ↦
              (1 : D) ⊗ₜ[K] ((algebraMap ℝ (InfiniteAdeleRing K) x) i) :=
            tensorPi_equiv_piTensor_apply ..
        _ = fun (i : InfinitePlace K) ↦ (1 : D) ⊗ₜ[K] (algebraMap ℝ i.Completion x) := by
            funext vi
            exact congrArg (fun z ↦ (1 : D) ⊗ₜ[K] z) (algebraMap_completion K)
        _ = (algebraMap ℝ ((i : InfinitePlace K) → D ⊗[K] i.Completion)) x := h₂.symm
    calc tensorPiEquivPiTensor K D InfinitePlace.Completion (x • y)
        = tensorPiEquivPiTensor K D InfinitePlace.Completion
            ((algebraMap ℝ (Dinf K D)) x * y) := congrArg _ (Algebra.smul_def x y)
      _ = tensorPiEquivPiTensor K D InfinitePlace.Completion ((algebraMap ℝ (Dinf K D)) x)
            * tensorPiEquivPiTensor K D InfinitePlace.Completion y :=
          tensorPi_equiv_piTensor_map_mul ..
      _ = (algebraMap ℝ ((i : InfinitePlace K) → D ⊗[K] i.Completion)) x
            * tensorPiEquivPiTensor K D InfinitePlace.Completion y :=
          congrArg (· * tensorPiEquivPiTensor K D InfinitePlace.Completion y) hkey
      _ = x • tensorPiEquivPiTensor K D InfinitePlace.Completion y := (Algebra.smul_def _ _).symm
}
...
lemma incl₂_isClosedEmbedding : Topology.IsClosedEmbedding (incl₂ K D) := by
  ...
  -- PORT (v4.33): `SetLike.setOf_mem_eq` is deprecated in favour of `SetLike.setOfPred_mem_eq`.
