...
import PhD.Main.QMF.FLTstuff.NumberField.Completion.Infinite
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.Module.Equiv
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Constructions
...
PORT (T016e-2) of `FLT.NumberField.InfiniteAdeleRing` (178 l.).
-/
...
instance [Algebra K∞ L∞]
  ...
  map_smul₂ a x := by
    ...
    | tmul l r =>
        funext w
        -- PORT (v4.33): `Completion.comapHom` is now built from `LiesOver.completionMap` rather
        -- than from `mapSemialgHom ∘ Isometry.mapRingHom`, so the last three unfolding simp
        -- lemmas of the original proof (`Isometry.mapRingHom`, `WithAbs.semialgebraMap`,
        -- `UniformSpace.Completion.mapSemialgHom`) are no longer needed.
