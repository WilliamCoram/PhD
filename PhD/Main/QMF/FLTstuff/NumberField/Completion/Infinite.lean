...
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.ContinuousAlgEquiv
import PhD.Main.QMF.FLTstuff.NumberField.InfinitePlace.Extension
...
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.Module.FiniteDimension
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.MetricSpace.Pseudo.Matrix
...
instance {v : InfinitePlace K} : NontriviallyNormedField v.Completion where
  non_trivial :=
    ...
    -- `UniformSpace.Completion.norm_coe` no longer matches; retype the norm first.
    ⟨x, by
      rw [show ‖((x : K) : v.Completion)‖ = ‖((WithAbs.toAbs v.1 x : WithAbs v.1) :
        v.1.Completion)‖ from rfl, UniformSpace.Completion.norm_coe]
      exact hx⟩
...
theorem denseRange_algebraMap_subtype_pi (p : InfinitePlace K → Prop) [NumberField K] :
  ...
  -- PORT (v4.33): `apply` no longer unifies the two (defeq) descriptions of the algebra map,
  -- so rewrite along the definitional equality first.
  have h : (⇑(algebraMap K ((v : Subtype p) → v.1.Completion))) =
      Subtype.restrict p ∘ ⇑(algebraMap K ((v : InfinitePlace K) → v.Completion)) := rfl
  rw [DenseRange, h]
...
abbrev comapHom (h : w.comap (algebraMap K L) = v) :
  ...
  -- PORT (v4.33): mathlib now provides `NumberField.LiesOver.completionMap` and
  -- `InfinitePlace.Completion` is a structure, so the `restrictScalars`/`mapSemialgHom`
  -- construction is no longer type-correct; upgrade `completionMap` to a `SemialgHom` instead.
  { __ := NumberField.LiesOver.completionMap (v := v) (w := w)
    map_smul' := by
      intro r x
      simp only [RingHom.toFun_eq_coe, Algebra.smul_def, map_mul]
      congr 1
      rw [show (algebraMap K v.Completion) r = ((WithAbs.toAbs v.1 r : WithAbs v.1) : v.Completion)
        from rfl, NumberField.LiesOver.completionMap_coe]
      rfl }

theorem comapHom_cont (h : w.comap (algebraMap K L) = v) : Continuous (comapHom h) :=
  -- PORT (v4.33): `UniformSpace.Completion.continuous_map` no longer applies to the new
  -- structure-based completion; use mathlib's continuity lemma for `completionMap`.
  have h' : w.1.LiesOver v.1 := ⟨by simp [← h, InfinitePlace.comap]⟩
...
theorem mem_placesOver_iff_comap (v : InfinitePlace K) (w : InfinitePlace L) :
  ...
  -- PORT (v4.33): `Set.mem_setOf_eq` is now the deprecated alias of `Set.mem_ofPred_eq`
...
theorem baseChangeEquiv_tmul (l : L) (x : v.Completion) :
  ...
  -- PORT (v4.33): `SemialgHom.baseChange_of_algebraMap_tmul` is now definitional, so `simp`
  -- flags it as an unused argument.
