...
Yaël Dillies
-/
import PhD.Main.QMF.FLTstuff.DedekindDomain.FiniteAdeleRing.BaseChange
import PhD.Main.QMF.FLTstuff.Mathlib.MeasureTheory.Constructions.BorelSpace.AdeleRing
import PhD.Main.QMF.FLTstuff.Mathlib.MeasureTheory.Constructions.BorelSpace.FiniteAdeleRing
import PhD.Main.QMF.FLTstuff.Mathlib.MeasureTheory.Constructions.BorelSpace.InfinitePlace
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.NumberField.FiniteAdeleRing
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.NumberField.InfinitePlace.Basic
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.Group.Quotient
-- PORT (T016e-3): `ContinuousAddEquiv.quotientPi` reaches `FLT.NumberField.AdeleRing`
-- transitively; in this tree it needs an explicit import.
import PhD.Main.QMF.FLTstuff.Mathlib.Topology.Algebra.ContinuousMonoidHom
import PhD.Main.QMF.FLTstuff.NumberField.InfiniteAdeleRing
import PhD.Main.QMF.FLTstuff.NumberField.Padics.RestrictedProduct
import Mathlib.NumberTheory.NumberField.AdeleRing
import Mathlib.NumberTheory.Padics.HeightOneSpectrum
import Mathlib.RingTheory.Flat.FaithfullyFlat.Basic
import Mathlib.RingTheory.Flat.TorsionFree
import Mathlib.RingTheory.SimpleRing.Principal
import PhD.Main.QMF.FLTstuff.Mathlib.Data.Real.Archimedean
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.NumberField.AdeleRing
import PhD.Main.QMF.FLTstuff.NumberField.Completion.Finite
...
PORT (T016e-3) of `FLT.NumberField.AdeleRing` (894 l.).
-/
...
noncomputable abbrev tensorProductEquivPi :
    L ⊗[K] (𝔸 K) ≃L[𝔸 K] (Fin (Module.finrank K L) → 𝔸 K) :=
...
theorem Rat.AdeleRing.integral_and_norm_lt_one (x : ℚ)
  ...
  -- PORT (v4.33): `InfinitePlace.Completion` is now a structure wrapping `v.1.Completion`, so
  -- `UniformSpace.Completion.norm_coe` only applies after peeling off `ofCompletion`, and the
  -- subsequent `change ‖(x : ℂ)‖ < 1` is no longer a defeq retype. Compute the norm instead.
  simp only [InfiniteAdeleRing.algebraMap_apply,
    NumberField.InfinitePlace.Completion.norm_ofCompletion, UniformSpace.Completion.norm_coe,
    WithAbs.norm_toAbs_eq, ← NumberField.InfinitePlace.coe_apply, Rat.infinitePlace_apply] at h1
  ...
  obtain ⟨y, rfl⟩ := intx
  -- PORT (v4.33): the norm now computes to `↑|x| < 1` with the absolute value taken in `ℚ`
  -- rather than to `|(x : ℝ)| < 1`, so cast down before splitting with `abs_lt`.
  norm_cast at h1 ⊢
...
theorem NumberField.AdeleRing.zero_discrete : ∃ U : Set (AdeleRing (𝓞 K) K),
  ...
  constructor
  · -- PORT (v4.33): `(piEquiv ℚ K).coe_toHomeomorph` now resolves to
    -- `Homeomorph.coe_toHomeomorph` (about `Equiv.toHomeomorph`), whose pattern is absent here;
    -- retype through `ContinuousAddEquiv.toHomeomorph` by `rfl` instead.
    rw [show ⇑(piEquiv ℚ K) = ⇑(piEquiv ℚ K).toHomeomorph from rfl, Homeomorph.isOpen_image,
      Set.ofPred_forall]
    apply isOpen_iInter_of_finite
    intro i
