...
import PhD.Main.QMF.FLTstuff.DedekindDomain.FiniteAdeleRing.TensorProduct
...
import PhD.Main.QMF.FLTstuff.Mathlib.LinearAlgebra.TensorProduct.Algebra
...
lemma FiniteAdeleRing.Aux.g_commSq {ι : Type*} [Fintype ι]
  ...
  intro x
  -- PORT (v4.33): the FLT proof ended with a `simp at f` hack; the goal is exactly
  -- `apply_symm_apply` for the continuous additive equivalence `f`.
...
noncomputable def φLocalKvLinear (v : HeightOneSpectrum (𝓞 K))
  ...
  map_smul' kv x := by
    ...
    induction x with
    -- PORT (v4.33): the FLT `simp only` lists no longer close these; both sides are
    -- `map_zero`/`map_add` images, so go through the induction hypotheses explicitly.
