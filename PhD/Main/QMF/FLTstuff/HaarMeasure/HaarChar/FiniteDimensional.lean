...
import PhD.Main.QMF.FLTstuff.Mathlib.LinearAlgebra.Matrix.Transvection
...
PORT (T016f-2a) of `FLT.HaarMeasure.HaarChar.FiniteDimensional`.
-/
...
lemma addEquivAddHaarChar_eq_ringHaarChar_det_matrix_pi [SecondCountableTopology F]
    (M : Matrix ι ι F) (hM : Pivot.ExistsListTransvecMulDiagonalMulListTransvec M)
    (h : IsUnit M.det) :
  ...
  apply diagonal_transvection_induction_of_det_unit P M hM h
  · -- diagonal case
    intro D h_ne hA
...
lemma addEquivAddHaarChar_eq_ringHaarChar_det_pi [SecondCountableTopology F]
  ...
  have hM : IsUnit M.det := by simp [M, e, LinearEquiv.isUnit_det']
