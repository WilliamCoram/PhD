...
import PhD.Main.QMF.FLTstuff.Mathlib.MeasureTheory.Constructions.BorelSpace.AdicCompletion
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.NumberField.AdeleRing
import PhD.Main.QMF.FLTstuff.NumberField.AdeleRing
...
import PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.FiniteDimensional
import PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.Padic
import PhD.Main.QMF.FLTstuff.HaarMeasure.HaarChar.RealComplex
import PhD.Main.QMF.FLTstuff.Mathlib.LinearAlgebra.Countable
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.NumberField.InfiniteAdeleRing
import PhD.Main.QMF.FLTstuff.Mathlib.NumberTheory.Padics.HeightOneSpectrum
...
import PhD.Main.QMF.FLTstuff.NumberField.HeightOneSpectrum
...
lemma MeasureTheory.ringHaarChar_adeles_rat (x : (𝔸 ℚ)ˣ) :
  ringHaarChar x = ringHaarChar (MulEquiv.prodUnits x).1 *
    (∏ᶠ p, ringHaarChar (RestrictedProduct.unitsEquiv _ (MulEquiv.prodUnits x).2 p)) := by
  unfold AdeleRing
  ...
  convert! addEquivAddHaarChar_restrictedProductCongrRight
    ...
    (fun p ↦
      (ContinuousAddEquiv.mulLeft (RestrictedProduct.unitsEquiv _ (MulEquiv.prodUnits x).2 p))) _
  exact (RestrictedProduct.unitsEquiv _ (MulEquiv.prodUnits x).2).2.mono
...
lemma MeasureTheory.ringHaarChar_adeles_units_rat_eq_one (x : ℚˣ) :
  ...
  · -- finite places
    ...
    simp only [RingHom.toMonoidHom_eq_coe,
      ...
      RestrictedProduct.unitsEquiv, RestrictedProduct.coeUnits, MonoidHom.comp_apply,
      MulEquiv.piUnits, RestrictedProduct.coeMonoidHom, MonoidHom.coe_mk, OneHom.coe_mk, RestrictedProduct.inv_apply,
