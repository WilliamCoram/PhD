...
import PhD.Main.NewtonPolygons.Spec
import PhD.Main.ForMathlib.NumberTheory.NewtonPolygon.PowerSeries
...
`Construction.lean` (packaged as `newtonPolygon₀OfSeq`) is the lower convex hull of the points
`(k, v k)` — the blueprint's claim "It should be clear that by construction this constructs the
lower convex hull of the set of points" (`blueprint/src/chapter/NP.tex`), made precise and
proved.
...
The proof of `height_le` ("below the points") is the segment-by-segment line bound — the
algorithm-level generalisation of `step_slope_le` from `Test/test.lean`. The proof of
`isGreatest` is the chord argument: a competitor is convex (`heightFun_chord`), sits below the
points, and the constructed polygon touches the points at its vertices, so on each segment the
competitor is below its own chord, which is below the constructed segment; final rays are
handled by an ε-approximation along slopes tending to the infimum.
-/
...
lemma IsNewtonPolygonOf.bddBelow {P : NewtonPolygon₀ (Γ := Γ)} (h : IsNewtonPolygonOf v P)
  ...
  refine ⟨NewtonPolygon.toReal (P.unitSlope 0), ?_⟩
  rintro s ⟨k', hk', -, a, hva, rfl⟩
...
`newtonPolygon₀OfSeq v`: its anchor is the first finite point, and segment `n` carries the slope
...
private lemma startingPoint_eq {i : ℕ} {c : Γ} (hff : findFirstFinite v 0 = some (i, c)) :
    (newtonPolygon₀OfSeq v).starting_point = ((i : ℤ), c) := by
...
private lemma exists_anchor (h1 : ∃ i, v i ≠ ⊤) :
    ∃ (i : ℕ) (c : Γ), findFirstFinite v 0 = some (i, c) ∧
      (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ) ∧ v i = (c : WithTop Γ) ∧
      ∀ k < i, v k = ⊤ := by
  classical
  obtain ⟨i, hi⟩ := h1
...
lemma newtonPolygon₀OfSeq_start_mem (h : ∃ i, v i ≠ ⊤) :
    ∃ k : ℕ, (k : ℤ) = (newtonPolygon₀OfSeq v).starting_point.1 ∧
      v k = ((newtonPolygon₀OfSeq v).starting_point.2 : WithTop Γ) := by
...
lemma newtonPolygon₀OfSeq_start_le (h : ∃ i, v i ≠ ⊤) :
    ∀ k : ℕ, (k : ℤ) < (newtonPolygon₀OfSeq v).starting_point.1 → v k = ⊤ := by
...
private lemma newtonPolygon_zero_eq {i : ℕ} {c : Γ} (hff : findFirstFinite v 0 = some (i, c)) :
    newtonPolygon v 0 = some (nextStep v i c) := by
...
private lemma newtonPolygon_succ_eq {n j₀ l : ℕ} {j₁ : Γ} {m : ℝ}
    ...
    newtonPolygon v (n + 1) = some (nextStep v j₀ j₁) := by
  simp only [newtonPolygon, h]

/-- The slope carried by segment `n` of the constructed polygon is the slope of step `n`. -/
private lemma newtonPolygon₀OfSeq_slopes {n : ℕ} {S : Step Γ} (h : newtonPolygon v n = some S) :
    (newtonPolygon₀OfSeq v).slopes n = slopes S := by
  show newtonPolygon_slopes v n = _
  simp only [newtonPolygon_slopes, h, slopes']

/-- The length of segment `n` of the constructed polygon is the length output by step `n`. -/
private lemma newtonPolygon₀OfSeq_lengths {n j₀ l : ℕ} {j₁ : Γ} {m : ℝ}
    (h : newtonPolygon v n = some (.nextVertex j₀ j₁ l m)) :
    (newtonPolygon₀OfSeq v).lengths n = ((l : ℕ) : WithTop ℕ) := by
  show newtonPolygon_lengths v n = _
  simp only [newtonPolygon_lengths, h]

/-- A final ray has infinite length, so it is followed by no vertex at all. -/
private lemma vertexX_succ_eq_top_of_ray {n : ℕ} {m : ℝ}
    (h : newtonPolygon v n = some (.limitingRay m) ∨ newtonPolygon v n = some (.infiniteRay m)) :
    (newtonPolygon₀OfSeq v).vertexX (n + 1) = ⊤ := by
  have hlen : (newtonPolygon₀OfSeq v).lengths n = ⊤ := by
    show newtonPolygon_lengths v n = ⊤
...
private lemma unitSlope_eq_of_mem_segment {i n i₀ t : ℕ} {m : ℝ}
    (hstart : (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ))
    (hvx : (newtonPolygon₀OfSeq v).vertexX n = ((i₀ : ℤ) : WithTop ℤ))
    (hslope : (newtonPolygon₀OfSeq v).slopes n = ((m : ℝ) : WithBotTop ℝ)) (hle : i ≤ i₀)
    (h1 : i₀ - i ≤ t)
    (h2 : (((i : ℤ) + t : ℤ) : WithTop ℤ) < (newtonPolygon₀OfSeq v).vertexX (n + 1)) :
    (newtonPolygon₀OfSeq v).unitSlope t = ((m : ℝ) : WithBotTop ℝ) := by
  refine ((newtonPolygon₀OfSeq v).unitSlope_eq_slopes ?_ (by rwa [hstart])).trans hslope
  rw [hvx, hstart]
...
private structure WalkInv (i n i₀ : ℕ) (i₁ : Γ) : Prop where
  ...
  vertexX_eq : (newtonPolygon₀OfSeq v).vertexX n = ((i₀ : ℤ) : WithTop ℤ)
  ...
  heightFun_eq : (newtonPolygon₀OfSeq v).heightFun (i₀ - i) = algebraMap Γ ℝ i₁
  /-- Every unit slope left of the vertex is honest. -/
  unitSlope_ne_top : ∀ t < i₀ - i, (newtonPolygon₀OfSeq v).unitSlope t ≠ ⊤
...
private lemma walkInv_succ {i n j₀ l i₀ : ℕ} {j₁ i₁ : Γ} {m : ℝ}
    (hstart : (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ)) (hInv : WalkInv v i n i₀ i₁)
    (hnp : newtonPolygon v n = some (.nextVertex j₀ j₁ l m))
  ...
  have hvx1 : (newtonPolygon₀OfSeq v).vertexX (n + 1) = ((j₀ : ℤ) : WithTop ℤ) := by
    have hmap : WithTop.map (fun l : ℕ => (l : ℤ)) ((l : ℕ) : WithTop ℕ) =
      (((l : ℤ)) : WithTop ℤ) := rfl
    rw [NewtonPolygon₀.vertexX_succ, hvx, newtonPolygon₀OfSeq_lengths v hnp, hmap,
      ← WithTop.coe_add]
    congr 1
    omega
  have hseg : ∀ t, i₀ - i ≤ t → t < j₀ - i →
      (newtonPolygon₀OfSeq v).unitSlope t = ((m : ℝ) : WithBotTop ℝ) := fun t h1 h2 =>
    unitSlope_eq_of_mem_segment v hstart hvx (newtonPolygon₀OfSeq_slopes v hnp) hik h1
      (by rw [hvx1]; exact WithTop.coe_lt_coe.2 (by omega))
  have hhf1 : (newtonPolygon₀OfSeq v).heightFun (j₀ - i) = algebraMap Γ ℝ j₁ := by
    have hsum := heightFun_add_of_unitSlope_eq (newtonPolygon₀OfSeq v) (m := m) (a := i₀ - i)
...
private lemma walk_step {i : ℕ} {c : Γ}
    ...
    (hvi : v i = (c : WithTop Γ)) : ∀ n : ℕ, newtonPolygon v n ≠ none →
      ∃ (i₀ : ℕ) (i₁ : Γ), WalkInv v i n i₀ i₁ ∧ newtonPolygon v n = some (nextStep v i₀ i₁) := by
  intro n
  induction n with
  | zero => exact fun _ => ⟨i, c, walkInv_zero v hff hvi, newtonPolygon_zero_eq v hff⟩
  | succ n ih =>
    intro hne
    obtain ⟨i₀, i₁, hInv, hnp⟩ := ih (newtonPolygon_ne_none_of_le v (Nat.le_succ n) hne)
    obtain ⟨p₀, p₁, l, m, hprev⟩ := newtonPolygon_nextVertex_of_succ_ne_none v hne
    have hstep : nextStep v i₀ i₁ = .nextVertex p₀ p₁ l m := Option.some.inj (hnp.symm.trans hprev)
    exact ⟨p₀, p₁, walkInv_succ v hstart hInv hprev hstep, newtonPolygon_succ_eq v hprev⟩

/-- The invariant at the vertex produced by a `nextVertex` step. -/
private lemma walkInv_out {n j₀ l : ℕ} {j₁ : Γ} {m : ℝ}
    (h : newtonPolygon v n = some (.nextVertex j₀ j₁ l m)) :
    ∃ i : ℕ, (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ) ∧ WalkInv v i (n + 1) j₀ j₁ := by
...
private lemma height_eq_at_vertex {i n i₀ : ℕ} {i₁ : Γ}
    (hstart : (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ)) (hInv : WalkInv v i n i₀ i₁) :
    (newtonPolygon₀OfSeq v).height (i₀ : ℤ) = ((algebraMap Γ ℝ i₁ : ℝ) : WithBotTop ℝ) := by
  have hle := hInv.le
  rw [show ((i₀ : ℤ)) = (newtonPolygon₀OfSeq v).starting_point.1 + ((i₀ - i : ℕ) : ℤ) by
      rw [hstart]; omega,
...
private lemma height_eq_segment {i n i₀ k : ℕ} {i₁ : Γ} {m : ℝ}
    (hstart : (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ)) (hInv : WalkInv v i n i₀ i₁)
    (hslope : (newtonPolygon₀OfSeq v).slopes n = ((m : ℝ) : WithBotTop ℝ))
    (hend : ∀ t < k - i, (((i : ℤ) + t : ℤ) : WithTop ℤ) < (newtonPolygon₀OfSeq v).vertexX (n + 1))
    (hik : i₀ ≤ k) :
    (newtonPolygon₀OfSeq v).height (k : ℤ) =
  ...
  have hne : ∀ t < k - i, (newtonPolygon₀OfSeq v).unitSlope t ≠ ⊤ := fun t ht => by
    rcases lt_or_ge t (i₀ - i) with h' | h'
    · exact hInv.unitSlope_ne_top t h'
    · rw [hseg t h' ht]; simp
  rw [show (k : ℤ) = (newtonPolygon₀OfSeq v).starting_point.1 + ((k - i : ℕ) : ℤ) by
      rw [hstart]; omega,
...
private lemma height_le_at_vertex {i n i₀ : ℕ} {i₁ a : Γ}
    (hstart : (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ)) (hInv : WalkInv v i n i₀ i₁)
    (hva : v i₀ = (a : WithTop Γ)) :
    (newtonPolygon₀OfSeq v).height (i₀ : ℤ) ≤ ((algebraMap Γ ℝ a : ℝ) : WithBotTop ℝ) :=
...
private lemma height_le_segment {i n i₀ k : ℕ} {i₁ a : Γ} {m : ℝ}
    ...
    (newtonPolygon₀OfSeq v).height (k : ℤ) ≤ ((algebraMap Γ ℝ a : ℝ) : WithBotTop ℝ) := by
...
private lemma height_le_aux {i : ℕ}
    ...
    ∀ (d n i₀ : ℕ) (i₁ : Γ), k - i₀ ≤ d → WalkInv v i n i₀ i₁ →
      newtonPolygon v n = some (nextStep v i₀ i₁) → i₀ ≤ k →
      (newtonPolygon₀OfSeq v).height (k : ℤ) ≤ ((algebraMap Γ ℝ a : ℝ) : WithBotTop ℝ) := by
  intro d
  induction d with
  | zero =>
...
theorem newtonPolygon₀OfSeq_height_le (h1 : ∃ i, v i ≠ ⊤) (h2 : IsAdmissible v) (k : ℕ) :
  ...
  | coe a =>
    ...
    · exact height_le_aux v hstart h2 hvk (k - i) 0 i c le_rfl (walkInv_zero v hff hvi)
        (newtonPolygon_zero_eq v hff) hik
...
private lemma tail_support_le {n : ℕ} (hnp : newtonPolygon v n = some Step.tail) :
    (newtonPolygon₀OfSeq v).support ≤ (n : WithTop ℕ) := by
  classical
  show newtonPolygon_numSegments v ≤ _
  unfold newtonPolygon_numSegments
...
private lemma tail_unitSlope_eq_top {i n i₀ : ℕ} {i₁ : Γ}
    (hstart : (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ)) (hInv : WalkInv v i n i₀ i₁)
    (hnp : newtonPolygon v n = some Step.tail) :
    (newtonPolygon₀OfSeq v).unitSlope (i₀ - i) = ⊤ := by
  have hle := hInv.le
  have hsupp := tail_support_le v hnp
  have hzero : ∀ n' : ℕ, n ≤ n' → (newtonPolygon₀OfSeq v).lengths n' = 0 := fun n' hn' =>
    (newtonPolygon₀OfSeq v).lengths_junk n' (hsupp.trans (by exact_mod_cast hn'))
  have hvxeq : ∀ n' : ℕ, n ≤ n' →
      (newtonPolygon₀OfSeq v).vertexX n' = ((i₀ : ℤ) : WithTop ℤ) := by
    intro n' hn'
  ...
  rcases (newtonPolygon₀OfSeq v).unitSlope_cases (i₀ - i) with ⟨n', hb1, hb2⟩ | htop | hbot
  · exfalso
    rw [show (newtonPolygon₀OfSeq v).starting_point.1 + ((i₀ - i : ℕ) : ℤ) = (i₀ : ℤ) by
      rw [hstart]; omega] at hb1 hb2
    rcases le_or_gt n n' with h' | h'
    · rw [hvxeq (n' + 1) (by omega)] at hb2
      exact lt_irrefl _ hb2
    · have hmono := (newtonPolygon₀OfSeq v).vertexX_mono (show n' + 1 ≤ n by omega)
      rw [hvxeq n le_rfl] at hmono
      exact lt_irrefl _ (hb2.trans_le hmono)
  · exact htop
  · -- a `⊥` unit slope would make the polygon a single `unboundedBelow` segment, but a `tail` at
    -- a positive index is preceded by a genuine `nextVertex` segment, whose slope is real
    exfalso
    obtain ⟨hb, h1⟩ :=
      (newtonPolygon₀OfSeq v).slopes_zero_eq_bot_of_unitSlope_eq_bot hbot
    have hn1 : 1 ≤ n := by
      have h' : (1 : WithTop ℕ) ≤ (n : WithTop ℕ) := h1 ▸ hsupp
      exact_mod_cast h'
    obtain ⟨p₀, p₁, l, mm, hp⟩ := newtonPolygon_nextVertex_of_lt v (show 0 < n by omega)
      (by rw [hnp]; exact Option.some_ne_none _)
    rw [newtonPolygon₀OfSeq_slopes v hp] at hb
    exact absurd hb (by simp [slopes])
...
private lemma height_eq_top_of_tail {i n i₀ k : ℕ} {i₁ : Γ}
    (hstart : (newtonPolygon₀OfSeq v).starting_point.1 = (i : ℤ)) (hInv : WalkInv v i n i₀ i₁)
    (hnp : newtonPolygon v n = some Step.tail) (hk : i₀ < k) :
    (newtonPolygon₀OfSeq v).height (k : ℤ) = ⊤ := by
  have hle := hInv.le
  by_contra hcon
  rw [show ((k : ℤ)) = (newtonPolygon₀OfSeq v).starting_point.1 + ((k - i : ℕ) : ℤ) by
    rw [hstart]; omega] at hcon
  exact (newtonPolygon₀OfSeq v).unitSlope_ne_top_of_height_ne_top hcon
...
private lemma isGreatest_aux {i : ℕ}
    ...
    ∀ (d n i₀ k : ℕ) (i₁ : Γ), k - i₀ ≤ d → WalkInv v i n i₀ i₁ →
      newtonPolygon v n = some (nextStep v i₀ i₁) → i₀ ≤ k →
      Q.height (k : ℤ) ≤ (newtonPolygon₀OfSeq v).height (k : ℤ) := by
  intro d
  induction d with
  | zero =>
  ...
  | succ d ih =>
    ...
    | limitingRay mm =>
      have hnp' : newtonPolygon v n = some (Step.limitingRay mm) := by rw [hnp, hs]
      rw [height_eq_segment v hstart hInv (newtonPolygon₀OfSeq_slopes v hnp')
        (fun t _ => by rw [vertexX_succ_eq_top_of_ray v (.inl hnp')]; exact WithTop.coe_lt_top _)
        hik]
    ...
    | nextVertex j₀ j₁ l mm =>
      ...
      have hle' := hInv.le
      rcases le_or_gt k j₀ with hkj | hjk
      · rw [height_eq_segment v hstart hInv (newtonPolygon₀OfSeq_slopes v hnp') (fun t ht => by
        ...
        congr 1
        ring
      · exact ih (n + 1) j₀ k j₁ (by omega) hInv1 (newtonPolygon_succ_eq v hnp') (le_of_lt hjk)
...
theorem newtonPolygon₀OfSeq_isGreatest (h1 : ∃ i, v i ≠ ⊤) (h2 : IsAdmissible v)
    ...
    (hle : ∀ k : ℕ, Q.height (k : ℤ) ≤ pointHeight v k) :
    Q.IsBelow (newtonPolygon₀OfSeq v) := by
  ...
  · obtain ⟨k, rfl⟩ : ∃ k : ℕ, x = (k : ℤ) := ⟨x.toNat, by omega⟩
    exact isGreatest_aux v hstart h2 hQ hle (k - i) 0 i k c le_rfl (walkInv_zero v hff hvi)
      (newtonPolygon_zero_eq v hff) (by exact_mod_cast hx)

/-- **Existence: the algorithm constructs the lower convex hull.** For an admissible sequence
with at least one point, the polygon `newtonPolygon₀OfSeq v` built by the step algorithm is the
Newton polygon of `v` in the sense of the geometric specification `IsNewtonPolygonOf`. -/
theorem isNewtonPolygonOf_newtonPolygon₀OfSeq (h1 : ∃ i, v i ≠ ⊤) (h2 : IsAdmissible v) :
    IsNewtonPolygonOf v (newtonPolygon₀OfSeq v) :=
  ⟨newtonPolygon₀OfSeq_start_le v h1, newtonPolygon₀OfSeq_start_mem v h1,
...
theorem isNewtonPolygonOf_powerSeries {R : Type*} [Semiring R] (val : R → WithTop Γ)
  ...
  isNewtonPolygonOf_newtonPolygon₀OfSeq (coeffSeq val f) h1 h2
