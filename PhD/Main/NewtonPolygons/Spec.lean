...
import PhD.Main.NewtonPolygons.Height
...
* `IsNewtonPolygonOf.unitSlope_zero_mul_le` — every point lies on/above the line of the first
  slope through the starting vertex (the spec-level "points on/above lines" workhorse).

Existence — the constructed polygon `newtonPolygon₀OfSeq v` satisfies the spec — is proved in
`SpecConstruction.lean`.
-/
...
/-- Contrapositive of `start_le`: every index carrying a finite value lies at or right of the
starting vertex. -/
lemma starting_point_fst_le (h : IsNewtonPolygonOf v P) {k : ℕ} (hv : v k ≠ ⊤) :
    P.starting_point.1 ≤ (k : ℤ) :=
  not_lt.1 fun hlt => hv (h.start_le k hlt)

/-- The anchor is determined by `v`: two Newton polygons of the same sequence start at the same
`x`-coordinate (the first finite index). -/
...
/-- Either Newton polygon of a sequence lies below the other. -/
...
`(x₀, y₀)` the starting vertex. This is the spec-level replacement for the algorithm-driven
`step_slope_le`/`firstBreak_slope_le` of `Test/test.lean`. -/
...
slope `m` (blueprint Definition "A polynomial is pure if its Newton polygon has only one
slope"). -/
