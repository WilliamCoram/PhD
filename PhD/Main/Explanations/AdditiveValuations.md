...
do. It supersedes the description of `PhD/Main/Test/addVals2.lean` (replaced 2026‑07‑30) and of the
single-file `PhD/Main/ForMathlib/RingTheory/Valuation/AddVal.lean` (split, same day).
...
(All paths relative to `PhD/Main/ForMathlib/`.)
...
`PhD/Main/ForMathlib/Analysis/Normed/Ring/NegLogNorm.lean`.
...
`PhD/Main/TateFredholm/00_Tate.lean` defines, for a multiplicative pseudo-uniformizer `ϖ`,
...
cosmetic defect. `PhD/Main/ForMathlib/NumberTheory/NewtonPolygon/Construction.lean` takes its input
...
`PhD/Main/TateFredholm/01_AddVal.lean` is a leaf that nothing depends on. It exists only to pin (2) and
(3) to (1):
...
* **`AddVal/Commensurable.lean` is 264 lines** and holds both the `ℚ`-valued theory and the
  ...
  natural second file.
* **`PhD/Main/Test/CompactOperatorsJohanssonNewton.lean`** still contains the old `ℝ`-valued
