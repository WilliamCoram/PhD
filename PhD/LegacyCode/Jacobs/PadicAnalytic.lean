...
* `Jacobs.padicLog u` — `log u = ∑ (-1)ⁿ (u - 1)^(n+1) / (n+1)`, for `‖u - 1‖ < 1`.
* `Jacobs.padicExp w` — `exp w = ∑ wⁿ / n!`, for `‖w‖` below the radius `‖3‖^(1/2)`.
* `Jacobs.unitPow t u` — `u^t := exp (t * log u)`, the weight-`t` power of a `1`-unit.
* `Jacobs.binomialCoeff t n`, `Jacobs.binomialSeries t e` — the binomial series
