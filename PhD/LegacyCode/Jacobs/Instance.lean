...
(`PhD.Jacobs.U3.Matrix`), which happens over Jacobs' own coefficient field `K₃ = ℚ₃`
...
(`PhD.Jacobs.U3.HeckeSlopes`).  But a reader should not conclude from this file that the
result needs an infinite-dimensional coefficient field.

## The cyclotomic certificate (not here — see `PhD.Jacobs.U3.HeckeSlopes`)
...
constructions.  This recipe is carried out in `PhD.Jacobs.U3.HeckeSlopes`, where the
