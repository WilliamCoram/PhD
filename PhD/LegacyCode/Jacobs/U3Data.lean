...
-/
import PhD.Jacobs.PadicAnalytic
import PhD.Jacobs.GenFun
...
* `Jacobs.eps*M*` — the eight matrices read off the `ε_{i,j}` displays (p. 28); the
...
* `Jacobs.weightGenFun t γ` — the generating function of `‖_κ γ` above.
* `Jacobs.h01 … h21` — the six block generating functions (2.1.4)–(2.1.9).
* `Jacobs.M22genFun`, `Jacobs.M22op` — the eigenblock
...
## The identification (PROVED — `PhD.Jacobs.U3.Matrix`)
...
* Theorem 2.1 (`D^×_f = ∐ D^× cᵢ U₁(9)`) — `Jacobs.U3.classRep_complete`, and
  Lemma 2.2 (`Γᵢ = 1`) — `Jacobs.U3.stabilizerAt_classRep`, both in `PhD.Jacobs.U3.ClassSet`;
* Lemma 2.3 (the decomposition of `Uη₃U`) — `Jacobs.U3.bijOn_etaRep`
  (`PhD.Jacobs.U3.EtaDecomposition`);
* Lemmas 2.4/2.5 and §B.1 (the nine factorisations `cᵢv_t⁻¹ = d·c(i,t)·u(i,t)`) —
  `Jacobs.U3.factorisation` and the tables `sigmaTable`/`dTable`/`uTable`
  (`PhD.Jacobs.U3.Factorisations`), *recomputed* by exhaustive search
  (`PhD/Jacobs/U3/certificate_search.py`), not transcribed;
* Proposition 2.6 (the generating function of the weight-`κ` action) —
  `Jacobs.U3.matrixCoeff_kappaOp` (`PhD.Jacobs.U3.KappaAction`), whose composition law
  needed the analytic-substitution development of `PhD.Jacobs.U3.Compose` and
  `PhD.Jacobs.BinomialTheorem`;
* the identification itself — `Jacobs.U3.heckeU3_apply_classRep` (unconditional) and
  `Jacobs.U3.eval_classRep_injective` (under the class-number-one hypothesis), in
  `PhD.Jacobs.U3.Matrix`.
...
decision record).  It is spectrally inert: `Jacobs.U3.charPowerSeries_blockOp_eq_U3MatrixOp`
proves that the assembled Hecke matrix and the transcribed `Jacobs.U3MatrixOp` have the
...
The endgame is also proved (board `.mathlib-quality/jacobs-endgame/`, 2026-08-06):
`det(1 − T·U₃)` is packaged as `Jacobs.U3.charPowerSeriesU3` with
`charPowerSeriesU3_eq_U3MatrixOp` (`PhD.Jacobs.U3.Fredholm`), and the eigenblock
factorisation `det(1 − T·U₃) = ∏ₜ det(1 − T·M_{t,t})` holds over every isometric
extension containing `ω` — `Jacobs.U3.charPowerSeriesU3_factorisation`, witnessed at
`L₃ = ℚ₃(ζ₃)` by `charPowerSeriesU3_factorisation_L₃` (`PhD.Jacobs.U3.HeckeSlopes`).
The one contracted external `sorry` is `hClassNumberOne` ([Jacobs, Lemma 1.22], the FLT
interface); every consumer carries it as an explicit hypothesis, and none of the
factorisation chain needs it.

## Main definitions

* `Jacobs.eps01M1 … Jacobs.eps21M`: the nine matrices read off the `ε_{i,j}` displays
  [Jacobs p. 28] — with `eps12M2`'s `a`-entry corrected, see the erratum note above.
* `Jacobs.weightGenFun`: the generating function `κ(cx+d)/((cx+d)(cx+d−axy−by))` of the
  weight-`κ` action [Jacobs, Proposition 2.6], taken here as the definition.
* `Jacobs.h01 … Jacobs.h21`: the six block generating functions (2.1.4)–(2.1.9).
* `Jacobs.M22genFun`, `Jacobs.M22op`: the `ω²`-eigenblock [Jacobs p. 34].

## Main results

* `Jacobs.norm_coeff_h02_le`, `Jacobs.norm_coeff_h12_le`: [Jacobs, Lemma 2.7] in
  quantitative form — the `xᵐyʳ` coefficient has norm `≤ ‖3‖ᵐ`, which is what makes the
  blocks compact.
* `Jacobs.lemma211_first`, `Jacobs.lemma211_first'`, `Jacobs.lemma211_second`,
  `Jacobs.lemma211_second'`: [Jacobs, Lemma 2.11]'s four commutation identities — the
  internal consistency check that caught the p. 28 misprint.
* `Jacobs.diagRescale_weightGenFun`, `Jacobs.weightGenFun_smul`: the substitution and
...
(p. 28); their derivation (Lemmas 2.3–2.5, §B.1) is proved in `PhD.Jacobs.U3.Factorisations`
...
is formalised in `Jacobs.U3.matrixCoeff_kappaOp` (`PhD.Jacobs.U3.KappaAction`), which
proves this series really is the generating function of `‖_κ γ` acting on the Tate
algebra, and `Jacobs.U3.sum_weightGenFun_eq_h` (`PhD.Jacobs.U3.Factorisations`) assembles
...
/-- `linSeries` written in monomial form. -/
...
/-- `quadSeries` written in monomial form. -/
...
/-- The constant coefficient of `linSeries γ` is `d = γ 1 1`. -/
lemma constantCoeff_linSeries {γ : Matrix (Fin 2) (Fin 2) K} :
    constantCoeff (linSeries γ) = γ 1 1 := by simp [linSeries]

/-- The constant coefficient of `quadSeries γ` is `d = γ 1 1`. -/
...
middle factor of `det(1 − T·U₃)` — see `Jacobs.charPowerSeries_U3MatrixOp`
(`PhD.Jacobs.DiamondW`) for the factorisation and `Jacobs.U3.heckeU3_apply_classRep`
(`PhD.Jacobs.U3.Matrix`) for the identification of `U₃`.  The *eigenspace* reading is
not formalised (it needs [Jacobs, Lemma 2.9]; see `PhD.Jacobs.DiamondW`'s header). -/
