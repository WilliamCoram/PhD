...
genuine Hecke operator: `Jacobs.U3.heckeU3_apply_classRep` computes `U₃ = [U₁(9)·η₃·U₁(9)]`
at the class representatives as the certificate block matrix, and
`Jacobs.U3.charPowerSeries_blockOp_eq_U3MatrixOp` shows that matrix has the same
characteristic power series as `U3MatrixOp` (both in `PhD.Jacobs.U3.Matrix`; the
formalised inputs Thm 2.1 – Prop 2.6 are listed in `PhD.Jacobs.U3Data`'s module header).
...
§B.2 factorisations) would need the certificate machinery of `PhD.Jacobs.U3.Factorisations`
...
`PhD.Jacobs.U3Data`, exactly as the thesis argues it ("since `U₃` and `W` commute, we
...
the matrix of `U₃`. -/
lemma isCompactoid_zero_clm (h3' : ‖(3 : K)‖ < 1) :
...
determinant twist: `Jacobs.U3.heckeU3_apply_classRep` and
`Jacobs.U3.charPowerSeries_blockOp_eq_U3MatrixOp` (`PhD.Jacobs.U3.Matrix`; module
header). -/
...
hence every slope statement of this file and `PhD.Jacobs.Slopes`, is identical for the
