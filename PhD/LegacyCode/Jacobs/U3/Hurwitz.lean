...
* `Jacobs.U3.IsHurwitz`: membership in the Hurwitz order, in halved-coordinate parity form.
* `Jacobs.U3.hurwitzOrder`: the Hurwitz order `𝓞 = ℤ⟨i, j, ½(1+i+j+k)⟩ ⊆ ℍ[ℚ]`.
* `Jacobs.U3.hnorm`: the reduced norm of a Hurwitz quaternion, as a natural number.
* `Jacobs.U3.ofTuple`, `Jacobs.U3.unitTuples`: the quaternion named by a halved-coordinate
  tuple, and the `24` tuples that name the units.

## Main results

* `Jacobs.U3.star_mem`: the order is closed under conjugation.
* `Jacobs.U3.hnorm_mul`: the integer norm is multiplicative.
* `Jacobs.U3.isUnit_iff_hnorm_eq_one`: `x` is a unit of `𝓞` iff `hnorm x = 1`.
* `Jacobs.U3.exists_unit_of_tuple`, `Jacobs.U3.exists_tuple_of_unit`: the unit group of `𝓞`
  is exactly the image of `unitTuples` under `ofTuple`.
* `Jacobs.U3.card_units_hurwitzOrder`: [Jacobs p. 22] — `𝓞^×` has exactly `24` elements.
