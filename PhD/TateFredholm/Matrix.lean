...
`[IsNoetherianRing R]`.  Bellaïche's claimed Noetherian-free equivalence is gapped at
Lemma II.1.8 (see `exists_truncation_near`'s docstring, the b2 log, and
`DECISION_noetherian_gap.md`). -/
...
/-- **[Bel] Lemma II.1.8 over a Tate ring**, corrected: finitely generated **closed**
submodules of `c_R(I)` are uniformly approximated by coordinate truncations.

The closedness hypothesis is necessary (b2 log, 2026-07-10): over the non-Noetherian
Banach–Tate ring `R = ℚₚ ⊕ c(ℕ, ℚₚ)` (pointwise multiplication) the principal submodule
generated by `g = (pʲeⱼ)ⱼ` contains the unit vectors `p⁻ⁿeₙ • g` concentrated at
coordinate `n`, so no single truncation approximates it uniformly.  [Bel]'s proof of
II.1.8 applies the open mapping theorem to `Aʳ → P`, which requires `P` Banach; per
[JN, p. 7] completeness of finitely generated submodules is precisely the Noetherian
input, and over a field it is automatic — so this statement still subsumes [Buz]/[Ser].
...
theory actually consumes, Noetherian-free.  (Restructure of 2026-07-10, b2 log: [Bel]'s
Noetherian-free equivalence with `IsCompletelyContinuous` is gapped, so the theory is
built on row decay; `IsCompactoid.isCompletelyContinuous` holds unconditionally, the
converse needs `[IsNoetherianRing R]` — see `DECISION_noetherian_gap.md`.) -/
...
-- The Noetherian bridge `IsCompletelyContinuous.isCompactoid` and the recovered
-- criterion `isCompletelyContinuous_iff_rowNorm` live in `Noetherian.lean` (they
-- consume the closedness of f.g. submodules; relocated 2026-07-10, see the b2 log).
...
sets.  (`hu` re-hypothesised from `IsCompletelyContinuous` per the 2026-07-10 b2 log.) -/
