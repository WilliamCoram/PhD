...
As in `PhD.Jacobs.U3Data` (see its module header): `M₂,₂` is *defined* by its generating
function; that it is the `ω²`-eigenblock of the Hecke operator `U₃ = [Uη₃U]` is assumed
(thesis Thm 2.1–Prop 2.6 and the `W`-eigenspace splitting, tracked as AG-B/AG-W in
`.mathlib-quality/jacobs/decomposition.md`).  The theorems here are unconditional about
the defined operators.

## Slope reading (deliberately NOT proved here)

The endpoint theorems compute the exact valuations `v₃(cₘ(M₂,₂)) = m²/2` — the *points*
`(m, m²/2)` of the Newton polygon of `det(1 − T·M₂,₂)` — but do **not** formalise the
slope statement: it remains to show that the Newton polygon defined by this valuation
sequence has slopes `1/2, 3/2, 5/2, …` (points on a strictly convex parabola ⇒ all
vertices ⇒ slopes = successive differences).  That is the separate AG-NP step (see
`PhD.Jacobs.SlopeTheorem`'s module header and the board), agreed to be worked
independently later.
-/
...
omit [CharZero K] in
/-- Operators on `c(ℕ, K)` are determined by their matrices: every `f` is the unconditional
sum `∑' i, f i • eᵢ` (`TateFredholm.cSpace.hasSum_single`), which a continuous linear map
transports term by term, so agreement on the canonical basis — that is, of all matrix
coefficients — is agreement. -/
theorem ext_matrixCoeff {u v : c(ℕ, K) →L[K] c(ℕ, K)}
    (h : ∀ j i, matrixCoeff u j i = matrixCoeff v j i) : u = v := by
  have hsingle : ∀ i : ℕ, u (cSpace.single i (1 : K)) = v (cSpace.single i (1 : K)) :=
    fun i => DFunLike.ext _ _ fun j => h j i
  refine ContinuousLinearMap.ext fun f => ?_
  have hu : HasSum (fun i => f i • u (cSpace.single i (1 : K))) (u f) := by
    simpa only [map_smul] using (cSpace.hasSum_single f).mapL u
  have hv : HasSum (fun i => f i • v (cSpace.single i (1 : K))) (v f) := by
    simpa only [map_smul] using (cSpace.hasSum_single f).mapL v
  exact hu.unique (by simpa only [hsingle] using hv)

omit [CharZero K] in
/-- The matrix of a scalar multiple (definitional). -/
private theorem matrixCoeff_smul (lam : K) (u : c(ℕ, K) →L[K] c(ℕ, K)) (j i : ℕ) :
    matrixCoeff (lam • u) j i = lam * matrixCoeff u j i := rfl
