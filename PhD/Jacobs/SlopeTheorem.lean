...
`(m, m(m−1)/2)` of the Newton polygon — but **do not themselves formalise the slope
statement**: one must still show that the Newton polygon defined by this valuation
sequence (its lower convex hull) has the wanted slopes — here, that the points lie on a
strictly convex parabola, hence are all vertices, hence the successive slopes are
`0, 1, 2, …`.  That polygon-level step is a separate development against
`ForMathlib.NumberTheory.NewtonPolygon` (seam AG-NP in
`.mathlib-quality/jacobs/decomposition.md`), agreed to be worked independently later;
the `val`-form theorem below is the exact input it will consume.
...
slopes `0, 1, 2, 3, …` — is the separate AG-NP step, not proved here. -/
...
This is the exact valuation-sequence input the Newton-polygon machinery
(`ForMathlib.NumberTheory.NewtonPolygon`) will consume; the reading "hence the slopes
are `0, 1, 2, 3, …`" is the separate AG-NP step (module header), not proved here. -/
