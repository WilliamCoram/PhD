...
The general (TateFredholm-flavoured) machinery for the AG-W tranche of the Jacobs board:
...
/-- **Matrix extensionality** (index-generic form of `Jacobs.ext_matrixCoeff`): operators out
of a model space are determined by their matrices, because every `f` is the unconditional sum
`∑' i, f i • eᵢ` (`cSpace.hasSum_single`), which a continuous linear map transports term by
term. -/
theorem ext_matrixCoeff [DecidableEq A] {u v : c(A, R) →L[R] c(B, R)}
    (h : ∀ j i, matrixCoeff u j i = matrixCoeff v j i) : u = v := by
  have hsingle : ∀ i : A, u (cSpace.single i (1 : R)) = v (cSpace.single i (1 : R)) :=
...
coordinate.  (The binder `(i : I)` is part of the fixed statement and intentionally
unused.) -/
@[simp] theorem matrixCoeff_inclSubtype (j : {x // p x}) (i : I) (f : c({x // p x}, R)) :
