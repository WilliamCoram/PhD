...
## Assumed input (identification debt — deliberately skipped)

Everything the thesis proves *before* the displays (2.1.4)–(2.1.9) is **assumed, not
formalised**: Theorem 2.1 (the class-set decomposition `D^×_f = ∐ D^× cᵢ U₁(9)`),
Lemma 2.2 (`Γᵢ = 1`), Lemma 2.3 (the decomposition of `Uη₃U`), Lemmas 2.4/2.5 and §B.1
(the nine factorisations `cᵢv_t⁻¹ = d·c(i,t)·u(i,t)`), and Proposition 2.6's derivation
of the generating function from the weight-`κ` action.  The matrices below are
*transcriptions* of the thesis's displays, and `weightGenFun` is Proposition 2.6's
closed formula taken as a *definition*.  Nothing in this file (or this tranche) proves
that these definitions are the matrix of the Hecke operator `U₃ = [Uη₃U]` on
`L(U₁(9), A₃)`; the theorems here are unconditional statements about the
explicitly-defined operators, and become statements about `U₃` only once that
identification is proved.  The identification is tracked as **AG-B** in
`.mathlib-quality/jacobs/decomposition.md` (adelic tranche, on `PhD/QMF/`; blocked on
QMF-board ports) and must be discharged before any "slopes of `U₃`" claim is final.
Partial mitigations already in place: Lemma 2.11's identities and Lemma 2.7's
integrality are *proved* from the transcribed data and machine-validate its internal
consistency (they caught one misprint in the thesis, see `eps12M2`).
-/
...
(p. 28) — see the module header: their derivation (Lemmas 2.3–2.5, §B.1) is assumed,
and their internal consistency is machine-checked by Lemma 2.7/2.11 below. -/
...
We take it as the **definition** of the block data — the thesis derives this formula
from the weight-`κ` action on the basis `e_r` ([Jacobs, proof of Prop. 2.6]), and that
derivation is part of the assumed input recorded in this file's header (identification
debt, AG-B): proving that `weightGenFun` really is the generating function of `‖_κ γ`
acting on the Tate algebra is future work, not done here.  Inverses are `MvPowerSeries`
field inverses — genuine two-sided inverses whenever `d ≠ 0`. -/
...
/-! #### The valuation table of `ν ≡ 2695 mod 3¹⁰` -/
...
/-! #### The three factors of `weightGenFun` -/
...
/-! #### Entrywise scaling of the matrix -/
...
/-! #### The scale factor `-8` of [Jacobs, Lemma 2.11]

Both chains of [Jacobs, Lemma 2.11] match their two sides through the entrywise scaling
...
/-- The operator `M₂,₂ : c(ℕ, K) →L[K] c(ℕ, K)`.  Per [Jacobs, pp. 32–34] this is the
block of `U₃` acting on the `ω²`-eigenspace of `W` in the topological basis `𝔅₂` — but
that identification is part of the assumed input (file header: identification debt,
AG-B/AG-W); here `M22op` is *defined* by its generating function `M22genFun`. -/
