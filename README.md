...
- `PhD/LWX/` — Liu–Wan–Xiao, *The eigencurve over the boundary of weight space* (arXiv:1412.2584v4): the halo ring `Λ^{>1/p}` and the universal character, the integral model of `U_p` and its entry streams (Prop 3.4), Theorem 3.16 / Corollary 3.18 on the halo estimate, and **Proposition 2.17 at every analyticity level** — `det(1 − X·U_p)` on `S^{D,†,m}` equals `det(I∞ − X·P)` in Colmez's basis (`SeamH.lean`, `QuaternionicH.lean`), which covers the whole boundary annulus `p⁻¹ < ‖T₀‖ < 1`.  The slope theorems (Thm 1.3 Step II, Thm 1.5 (1.5.1), both conditional on the touching hypothesis) are read on the genuine `U_p` through that seam in `SlopesSeam.lean`.  Boards: `.mathlib-quality/lwx-halo/`, `lwx-slopes/`, `lwx-seam/`, `lwx-seam-m/`.
